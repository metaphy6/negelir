...
class TrainingPipeline:
    ...
    def run(
        ...
        *,
        ...
        # Phase 22.4: training_report moved to docs/reports/ai_pipeline/
        import importlib.util
        import sys
        import os as os_module
        spec = importlib.util.spec_from_file_location(
            "training_report",
            os_module.path.join(os_module.path.dirname(__file__), "..", "docs", "reports", "ai_pipeline", "training_report.py")
        )
        training_report = importlib.util.module_from_spec(spec)
        sys.modules["training_report"] = training_report
        spec.loader.exec_module(training_report)
        StageBundle = training_report.StageBundle
        write_report = training_report.write_report
