...
- [ ] **Tier-1 (security / integrity / money / consensus).** Modules
...
- [ ] Python coverage runs with `--branch` (a new `.coveragerc` /
...
- [ ] **Every PR's changed lines** meet their tier floor
      ...
      code, always").
- [ ] A bug-fix PR must add a line that was **red before** the fix
...
- [ ] **`mutmut` (Python) / `go-mutesting`-class (Go)** run nightly on
...
- [ ] **No coverage-pragma abuse.** `# pragma: no cover` /
      `//coverage:ignore` require an inline justification and are
      counted; a budget cap (`cfg.coverage_pragma_max_per_module`)
      prevents silently excluding hard paths.
- [ ] **Coverage cannot fall.** A ratchet (`make coverage.ratchet`)
...
- [ ] `make coverage.report` (line+branch, HTML+JSON artifact),
...
- [ ] Coverage config + tier map land under single-source config
