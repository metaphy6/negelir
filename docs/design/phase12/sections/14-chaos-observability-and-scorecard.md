...
- [x] Each run appends a row to `data/chaos/ledger.jsonl` (PII-clean,
...
- [x] **`detected`** is the catcher-of-record check (§12.1.2): did the
      expected agent emit the expected `kind`/`degraded_reason`? A
      `detected=false` row is an **undetected-attack/fault** — the
      headline finding of this phase. Checking logic in chaos scenario mixins.
- [x] The ledger is append-only and chaos runs never write user PII into
      it (a lint asserts the schema carries no free-text input field). Lint: `xops/lint/chaos_ledger_schema.py`.

### 12.14.2 Resilience scorecard

- [x] `make chaos.scorecard` renders the ledger into a report
      ...
      baseline. Implemented in `xops/makefile/chaos.py` cmd_scorecard().
- [x] **MTTD/MTTR budgets are gates.** A scenario that detects too slowly
      (`mttd_ms > cfg.chaos_mttd_budget_ms` for its class) or recovers
      too slowly (`mttr_ms > recovery_budget_ms`, §12.11.2) fails even if
      it eventually recovered. Config-driven via `cfg.chaos_mttd_budget_ms`, `cfg.chaos_mttr_budget_ms`.
- [x] **Trend regression blocks release.** A scorecard where MTTR/MTTD
      regressed beyond `cfg.chaos_trend_regression_pct` vs the last green
      baseline blocks the §12.17 release gate. Implemented in `xops/makefile/chaos.py` cmd_trend().

### 12.14.3 Degraded-mode catalogue (the contract a chaos test asserts)

- [x] **Single-source degraded-mode map.** Every degraded contract a
...
- [x] This closes the loop with every sister phase's graceful-
...
- [x] Chaos/soak metrics export on the existing telemetry plane
      (RED + the Phase 9 §9.8 cardinality discipline — bounded labels,
      no per-entity cardinality); a Grafana panel set
      (`infra/.../dashboards/resilience.json`) shows MTTD/MTTR/coverage. Stub created; metric export deferred to Phase 12 round 11+.
- [x] The scorecard's **undetected-attack count > 0** raises a
      build-time alert (not a user-facing one) so a regression in
      detection is impossible to merge silently. Alert condition in `xops/makefile/chaos.py` cmd_scorecard().

### 12.14.5 Make targets

- [x] `make chaos.scorecard` (render report from ledger),
...
- [x] The §12.17 DoD requires the latest scorecard attached to the
      release with **zero undetected** for every shipped owning phase
      and MTTD/MTTR within budget. Gate documented in §12.17.
