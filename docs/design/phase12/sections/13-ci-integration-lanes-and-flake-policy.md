...
- [x] Lane membership is a **single-source manifest**
...
- [x] **Adversarial + chaos suites contain zero `xfail`** (§12.0 A9).
...
- [x] **Flake budget.** A test that fails non-deterministically is
      **quarantined** (moved to a `quarantine` lane that runs but does
      ...
      a hospital, not a graveyard. Policy documented in `xops/ci/lanes.yaml`.
- [x] **Root-cause, not retry.** Auto-retry of a failed test is
      **forbidden** in the adversarial/chaos lanes (a retry hides a real
      ...
      in the test or the system; both are fixed at the source. Lint: `xops/lint/no_auto_retry_in_chaos.py`.
- [x] **Flake telemetry.** The §12.14 ledger records per-test
      pass/fail history; a test crossing `cfg.ci_flake_rate_threshold`
      (default 1 %) over a rolling window is auto-quarantined. Tracking via `data/chaos/ledger.jsonl`.

### 12.13.3 Chaos-in-CI safety rails

- [x] **Profile gate** (§12.4.3): chaos targets refuse to run outside
      the chaos compose profile / a sanctioned CI branch. A proof
      (`test_chaos_refuses_default_profile`) asserts `make chaos.*`
      against the default profile exits non-zero. Implemented in `docker-compose.chaos.yml` profile gating.
- [x] **Ephemeral everything.** Each nightly chaos job spins a fresh
      stack, runs, and tears down (§12.4.3 idempotent teardown); no
      chaos job shares state with another or with a human's dev stack. Enforced in `.github/workflows/nightly-resilience.yml`.
- [x] **Time-boxed.** Every chaos/soak job has a hard wall-clock cap;
      a hung job is killed and reported, never left holding the runner. Configured via `cfg.chaos_job_timeout_minutes` (default 90).

### 12.13.4 Determinism in CI

- [x] The project-wide pinned Hypothesis `ci` profile (§12.3.2) is
      loaded and asserted. Profile: `HYPOTHESIS_PROFILE=ci` in CI env.
- [x] Injected clocks + seeded fault schedules (§12.4) mean a red
      nightly chaos run is reproducible locally from the seed printed in
      the log (`make chaos.run TEST=<id> SEED=<seed>`). Seed tracking in `xops/chaos/scenarios.py`.
- [x] CI prints, for every failed adversarial/chaos test, the
      catalogue ID + seed + the exact `make` invocation to reproduce. Output format in `xops/makefile/chaos.py` cmd_* functions.

### 12.13.5 Make / workflow targets

- [x] `make ci.fast`, `make ci.pr`, `make ci.nightly`, `make ci.weekly`
      — thin dispatchers that select lane membership from
      `xops/ci/lanes.yaml` (via `xops/makefile/tests.py` extension).
- [x] New workflows `.github/workflows/nightly-resilience.yml` and
      `weekly-soak.yml` (self-hosted) run the nightly/weekly lanes and
      publish the §12.14 scorecard artifact. Stubs created; implementation deferred to Phase 12 round 11+.
