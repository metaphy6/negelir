...
- [x] **`make chaos.list` reads the catalogue**, not a hand-maintained
      duplicate; a lint (`xops/lint/chaos_catalogue_sync.py`) asserts
      every `make chaos.*` target has a catalogue row and vice-versa.

### 12.5.1 ID scheme

- [x] **Stable prefix `P12-<phase>-<seq>`.** The owning phase is encoded
...
- [x] **Canonical target name is dot-style** (§12.0 A2):
...
- [x] **Mis-prefix fix.** The §10-era stubs `P12-8-AM…AT` describe NLP
...
- [x] **Reconcile, do not invent.** Every added row points at an
...
- [x] **Fix the malformed row** — the merged `… || P12-8-AM` cell at the
      end of the §8 table is split into two well-formed rows.
- [x] **Re-frame the header** from "Phase 7 stubs" to "cross-phase
      Phase 12 registry" with the §12.5.1 ID scheme + the old→new target
      naming map.
- [x] **Add the missing families** (`P12-5-*`, `P12-9-*`, `P12-11-*`,
...
- [x] **Normalise target names** to dot-style without changing any ID.
- [x] `make verify.chaos-catalogue` (new) asserts: no malformed rows,
...
- [x] Once an owning phase has shipped, **every** `status=implemented`
      stub for it must be green; a regression flips the rollup red.
- [x] An owning phase may **not** be marked complete in its own DoD
