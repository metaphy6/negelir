...
- [x] The same scenario ID (§12.5) runs at **both** planes where
...
- [x] **Deterministic schedule.** A fault schedule is
...
- [x] **Named, not magic.** Every injected fault carries a `name` that
      appears in the test and in the §12.14 run ledger, so a failure is
      traceable to the exact fault that caused it.
- [x] **Production-safe.** `test_fault_injector_is_noop_in_prod` asserts
      ...
      `xops/chaos/`.
- [x] **Blast-radius containment.** Injected faults only touch the
...
- [x] **New `docker-compose.chaos.yml`** layers Toxiproxy between every
      ...
      pattern.
- [x] **Toxiproxy toxics** map 1:1 onto `FaultInjector` ops: `latency`,
...
- [x] **Pumba scope.** Container-lifecycle chaos (`chaos.kill-predictor`,
      ...
      safety rail).
- [x] Tool versions pinned in `xops/versioning/chart.json` compatibility
      (Toxiproxy, Pumba images by digest — no `*-latest`, per CLAUDE.md).

### 12.4.3 Safety rails (chaos must not become an outage)

- [x] **Profile gate.** Chaos targets refuse to run unless
...
- [x] **Auto-heal timeout.** Every injected fault carries a
...
- [x] **Idempotent teardown.** `make chaos.down` is idempotent and
...
- [x] `make chaos.up` / `make chaos.down` — bring the chaos compose
      profile up/down.
- [x] `make chaos.run TEST=<id>` — run one catalogue scenario by stable
      ID (§12.5) at the realistic plane; emits a §12.14 ledger row.
- [x] `make chaos.list` — print the catalogue (single source:
      [`docs/testing/phase12_catalogue.md`](../../../testing/phase12_catalogue.md)).
- [x] `make test.chaos.inproc` — run the in-process `FaultInjector`
