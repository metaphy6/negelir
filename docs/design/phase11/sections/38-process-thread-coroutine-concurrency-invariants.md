...
- [x] **Router threading model.** Router state (recent p95s, sticky map, matrix pointer) is read-mostly; updates use copy-on-write atomic-pointer swaps. Sticky map is a bounded `OrderedDict` behind a `threading.Lock` (≤ 1 µs critical section). No request waits on another request's router work.
- [x] **No fork-after-import-torch.** §11.3 already bans fork after CUDA init; this extends to **all** GPU-backend imports (torch, openvino, xgboost-gpu, tensorrt). The supervisor sets `multiprocessing.set_start_method("spawn", force=True)` at the earliest possible point; tested.
- [x] **GIL hot-loop discipline.** The continuous-batch scheduler (§11.22) and the router's score loop (§11.13) are pure-Python on the request hot path; both are profiled to keep their per-call cost ≤ `cfg.compute_router_p99_overhead_us` (default 200 µs) so the GIL never becomes the bottleneck. Regression gated in §11.9 bench.
