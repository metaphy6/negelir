...
- [x] **`Retry-After` bounds.** Computed from the arbiter's queue model (`expected_wait = qd × p95_recent`) clamped to `[cfg.api_retry_after_min_s, cfg.api_retry_after_max_s]`. Never `0`, never unbounded.
- [x] **`X-Compute-Reason` enum.** Closed enum, documented in `docs/design/COMPUTE_DEVICES.md`; lint refuses ad-hoc strings.
- [x] **Tested end-to-end.** A Phase 12 chaos scenario forces each row above and asserts the exact triple `(status, Retry-After, X-Compute-Reason)`.
