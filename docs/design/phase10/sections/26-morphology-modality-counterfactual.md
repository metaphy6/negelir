...
- [x] **Latency budget unchanged.** Voice path adds ≤ 3 ms p95 to
  §10.1 normalize stage (filler strip is one regex pass + table
  lookup). CI bench gate enforces.
- [x] **Adversarial corpus extension.** §10.18 evaluation harness
...
- [x] **Why this matters.** Turkish mobile users split between Q
...
- [x] **Layout-aware confusable matrix.** `ai/nlp/lang_tr/ime/keyboard_confusables.tr.yaml`
...
- [x] **Layout detection (heuristic, advisory).** Optional
...
- [x] **Symspell extension.** `ai/nlp/vendor/symspell.py` gains
...
- [x] **Autocorrect-cascade detection.** Closed table
...
- [x] **Per-query layout-aware budget.** `cfg.nlp_typo_max_lookups_per_query`
...
- [x] **Privacy.** `keyboard_hint` and `client_id`-keyed layout
...
- [x] **Number-word ↔ digit bidirectional resolver.** §10.22.6
  ...
  *"2024 yılında"* and *"iki bin yirmi dört yılında"* must collapse
  ...
  generated form.
- [x] **Ordinal handling.** Closed forms only:
  ...
  *hafta/maç*). Deterministic per closed table; never
  ML-predicted.
- [x] **Fractional time.** `yarım/çeyrek/buçuk` resolver:
...
- [x] **Relative time enrichment.** Add to §10.5 date resolver:
...
- [x] **DST-bridging clock arithmetic.** When relative time crosses
  ...
  (debounced).
- [x] **Numeric format ambiguity (extends §10.24.11).** `1.234`
...
- [x] **Half-built number guard.** A user typing *"iki bin"* and
...
- [x] **Real bug.** §10.4 intent enum has no slot for hypothetical-
...
- [x] **Modal-aspect detector (deterministic, table-driven).** NEW
...
- [x] **Detection algorithm.** §10.5 entity pass gains
...
- [x] **Routing override (binding).** §10.6 dispatcher MUST NOT
  ...
  - `counterfactual_past` → `meta.counterfactual_past_unsupported`
...
- [x] **Ambiguous modality.** When the modality classifier returns
...
- [x] **Proof-test density.** ≥ 30 modality cases curated by hand
...
- [x] **Real attack vector.** §10.25.6 added two-reviewer rule for
...
- [x] **PR diff size cap.** `make verify.nlp-lexicon-diff` (NEW;
...
- [x] **Typo-squat detector.** For every newly added alias, run
...
- [x] **Ortho-confusable canonical guard.** `make verify.nlp-lexicons`
...
- [x] **Alias age + provenance.** §10.25.6 added `source` field;
...
- [x] **Lexicon CI-bot impersonation defense.** The PR-flood
...
- [x] **Real gap.** §10.25.1 multi-turn conversation persists
...
- [x] **Correction-utterance detector.** Closed table
...
- [x] **Detection trigger + scope.** Detector fires only when
...
- [x] **Context-update semantics.** On detection:
...
- [x] **Audit trail.** Every correction emits an audit row with
...
- [x] **Real gap §10.21.8 left open.** §10.21.8 added an HMAC over
  ...
  legitimate citation.
- [x] **`qa.answer.v1.envelope_signature` (additive,
  ...
  cannot read it).
- [x] **`body_canonical` derivation.** NFC-normalize body, replace
...
- [x] **Real-world input.** Users frequently paste a paragraph
  ...
  *control/zero-width/RTL*, not whitespace variants), contains
  ...
  Twitter/X)`).
- [x] **§10.1 step 3 widening.** Strip set extends to include:
  ...
  - Dash canonicalization: `\u2013\u2014\u2212 → -` (en-dash,
    em-dash, minus).
  - Ellipsis: `\u2026 → ...` (3 dots).
- [x] **Determinism + idempotency.** All extensions tested by
...
- [x] **Length-cap ordering.** Strip extensions run AFTER length
...
- [x] **Citation-tail stripper (closed enum).**
...
- [x] **Real gap.** §10.22.8 added a bilingual gazetteer pass for
  ...
  miss intent.
- [x] **Closed football vocab table.** `lang_tr/football/vocab.tr.yaml`
  ...
  tur atlama).
- [x] **Resolver behaviour.** Each surface form maps to
  ...
  per §10.4 floor.
- [x] **Bilingual abuse-token guard.** Some English football
...
- [x] **Build pipeline + governance.** Same two-reviewer
...
- [x] **Real gap.** §10.24.13 handled the per-request empty-input
...
- [x] **Per-subject empty-input rate gauge.** New
...
- [x] All `[ ]` items in §10.26.1–§10.26.13 ticked.
- [x] **§10.20 DoD item 24 added** — "All §10.26 ticks required."
- [x] **~72 new proof tests** distributed:
...
- [x] **`make swarm.demo.nlp.full` extends** within ≤ 60s budget
...
- [x] **Chart compatibility additions.** Pin
...
- [x] **Documentation.** `docs/design/TURKISH_NLP.md` gains
...
- [x] **Cross-phase contract updates.**
