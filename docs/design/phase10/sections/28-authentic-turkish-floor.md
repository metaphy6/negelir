...
- [x] **Real bug.** Football-specific bilingual vocab (§10.26.10) covered the
  ...
  *"offside"*, *"ofsayd"*, *"ofsayde"*). Symspell with edit-distance ≤ 2 over a
  ...
  from the canonical.
- [x] **Closed table** `ai/nlp/lang_tr/loanwords/transliteration_variants.tr.yaml` —
...
- [x] **AST guard `test_nlp_loanword_variants_dont_collide_across_domains`** —
...
- [x] **Proof tests.** ≥ 40-row golden `loanword_corpus.tr.json` with 4-8 variants
  per canonical for football terms; 100% gate.

#### 10.28.6 Fragment / incomplete-sentence detection

- [x] **Real bug.** Users hit Send by mistake or autocorrect drops the last word:
...
- [x] **`ai/nlp/lang_tr/fragments/`** — closed structural-pattern detector:
...
- [x] **Routing.** Fragment → closed `meta.fragment_detected.<locale>.j2` template
...
- [x] **False-positive guard.** Closed allow-list for legitimate fragment-shaped
  ...
  100% pass-through.
- [x] **Proof tests.** ≥ 25 fragment positives + 30 negative; FP rate ≤ 3%.

#### 10.28.7 Distractor-preamble stripping

- [x] **Real bug.** Voice / paste / mobile users routinely prepend distractor
...
- [x] **`preamble_strippers.tr.yaml`** — closed phrase set (≤ 200 entries):
  ...
  patterns).
- [x] **New §10.1 step 6.7** `strip_preamble` AFTER ASR-filler strip (§10.26.2)
...
- [x] **Hard guard.** Never strip if the resulting tail has < 2 tokens (would
  collapse a vocative-only message to empty; §10.24.13 owns that path). AST
  `test_nlp_preamble_does_not_collapse_to_empty`.
- [x] **Audit invariant.** Original full text preserved in spool / audit rows
  per §10.21.7 PII discipline; only the *classifier input* is the stripped form.
- [x] **Proof tests.** ≥ 25-row corpus (preamble + real question pairs) — assert
...
- [x] **Real bug.** *"GALATASARAY KAZANIR MI BU HAFTA?"* — fully valid TR question,
...
- [x] **`detect_all_caps(text) -> bool`** — ratio of uppercase letters to total
...
- [x] **Telemetry.** `nlp_input_shout_total` counter; over-pressure
...
- [x] **Output policy.** NEVER mirror shouting in the answer; templates are
  fixed-case. AST `test_nlp_template_renderer_does_not_propagate_shout` asserts no
  template branches on `shout`.
- [x] **Proof tests.** ≥ 15-row corpus (clean shouts that should still resolve), ≥
...
- [x] **Real bug.** TR `ğ` is hard to type on some IMEs / disappears in fast typing /
...
- [x] **Closed `g_to_softg.tr.yaml`** — corpus-derived list of words where `g→ğ`
...
- [x] **Pipeline integration.** §10.1 step 6 (diacritic restore) extends with a
...
- [x] **Proof tests.** ≥ 30-row corpus; CI 100%; ≥ 15-row negative (words like
  *"gol"*, *"galatasaray"*, *"genç"* must NOT have `g→ğ` applied).

#### 10.28.10 Meta / rhetorical / non-answerable question routing

- [x] **Real bug.** *"Sen ne biliyorsun ki?"*, *"yardım edebilir misin?"*,
...
- [x] **`meta_questions.tr.yaml`** — closed enumeration of rhetorical /
...
- [x] **Routing override.** Detection happens AFTER §10.26.5 modality firewall but
  ...
  into `predict.*`).
- [x] **Tier-blind.** Refusal templates are tier-blind (free + paid both get the
  polite refusal). AST `test_nlp_meta_questions_tier_blind`.
- [x] **Proof tests.** ≥ 40-row golden across 3 classes; CI 100%; ≥ 15-row
...
- [x] **Real bug.** Worst-case combinatorial blow-up: §10.1 normalize + §10.3
...
- [x] **`ai/nlp/runtime/budget.py`** — `RequestBudget` context manager:
...
- [x] **Default budgets** (cfg-tunable). `nlp_per_request_cpu_budget_ms=200`
...
- [x] **Budget exhaustion → degraded path.** `BudgetExceeded` at any pipeline
...
- [x] **Subprocess discipline.** Humanizer (§10.25.11) runs in a subprocess
...
- [x] **Proof tests.** *(a)* `test_nlp_runaway_normalize_cpu_budget_kicks` —
...
- [x] **Real bug.** §10.21.2 specifies "Symspell rebuilt-not-mutated on swap" but
...
- [x] **`nlp_lexicon_rebuild_concurrency_max=2`** — at most 2 of {teams, players,
...
- [x] **Rebuild RSS reservation.** Each in-flight rebuild reserves
...
- [x] **Per-request budget exception during swap.** Requests landing during an
...
- [x] **Proof tests.** *(a)* `test_nlp_lexicon_concurrent_rebuild_capped_at_two` —
...
- [x] **Real bug.** §10.4 abstention floor + §10.5 entity ambiguity each fire
...
- [x] **`SkewDetector`** in `nlp.dispatcher.v1`: per-decision compute
...
- [x] **Telemetry only at v1.** Histogram `nlp_classifier_extractor_skew` per
...
- [x] **Proof tests.** *(a)* `test_nlp_skew_detector_fires_on_synthetic_skew` —
...
- [x] **Phase 4 (storage).** §10.28.4 PII redaction runs BEFORE storage of any
  ...
  TR-PII regex set.
- [x] **Phase 5 (calibration).** Skew detector (§10.28.13) emits histogram
  consumed by Phase 5.x retraining trigger heuristic; not a hard dependency, no
  schema change at v1.
- [x] **Phase 7 (sec layer).** TR-PII detector module (`tr_pii.py`) is
...
- [x] **Phase 8 (patcher).** Patcher scope EXCLUDES `ai/nlp/lang_tr/spelling/`,
...
- [x] **Phase 9 (gateway).** Gateway honours the `request_metadata.shout`
  flag forwarded in `qa.intent.v1` for telemetry only; no header surface.
- [x] **Phase 11 (compute).** §10.28.11 budget module is CPU-only; AST guard
...
- [x] **Phase 13a (LeagueCatalog).** §10.28.1 consonant-alternation table
...
- [x] **Phase 14 (K8s).** No new probes; §10.28.11 budget exhaustion does NOT
  fail liveness (degraded path is graceful per §10.10). Readiness is unaffected.
- [x] **Phase 16 (Emitter).** `LexiconStore` Protocol unchanged; new tables
...
- [x] **Phase 19 (long-tail leagues).** All §10.28 tables grow additively per
  league; AST `test_nlp_no_per_league_branch` re-asserts on the new code surface.
- [x] **Phase 20 (monetization).** All §10.28 paths are tier-blind (orthographic
...
- [x] **~24 new cfg knobs** (additive, documented in `xops/env/.env.example`,
  ...
  `nlp_per_request_cpu_budget_with_humanizer_ms=800`,
...
- [x] **New `nlp.event.v1` kinds**: `consonant_softening_repaired`,
...
- [x] **New `nlp.alert.v1` kinds**: `nlp_pii_in_input_tc_kimlik`,
...
- [x] **~70 new proof tests** on top of §10.20+§10.21+§10.22+§10.23+§10.24+§10.25+§10.26+§10.27
  ...
  sub-section).
- [x] **`make nlp.transliteration-build`** (NEW xops target) regenerates
  loanword-variants table; CI gate refuses PR diffs > 50 rows without
  `nlp-curator` CODEOWNER ack.
- [x] **CODEOWNERS additions**: every new YAML under `ai/nlp/lang_tr/spelling/`,
  `ai/nlp/lang_tr/loanwords/`, `ai/nlp/lang_tr/fragments/`,
  `ai/nlp/lang_tr/preamble_strippers.tr.yaml`,
  `ai/nlp/lang_tr/meta_questions.tr.yaml`,
  ...
  CI-gated.
- [x] **Versioning.** `swarm` minor (or patch if no public surface change),
...
- [x] **Tracker row + ROADMAP checkbox flips** per AGENTS.md §3 + §3.4 — every
