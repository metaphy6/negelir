...
- [x] **Fix.** Closed `ai/nlp/lang_tr/spelling/geminate_restoration.tr.yaml`
  ...
  single canonical id.
- [x] **Build refusal.** `make verify.nlp-lexicons` AST-asserts every
...
- [x] **Proof tests.** ≥30-row positive golden (hak→hakkı, sır→sırrı, his→
...
- [x] **The problem.** Disyllabic stems with a high vowel in the second
...
- [x] **Fix.** Closed `ai/nlp/lang_tr/spelling/vowel_drop_before_suffix.tr.yaml`
...
- [x] **LeagueCatalog build refusal.** Any player/manager surname ending
  in `{Cl,Cn,Cr,Cz,Cm}` clusters where C is a stop must have a decision
  row (`drops|invariant`).
- [x] **Proof tests.** 25-row positive + 10-row negative (gol→golü never
...
- [x] **The problem.** Stems ending in `-nk` soften the `k` to `g` (not
...
- [x] **Fix.** Extend `consonant_alternations.tr.yaml` with `kind=nk_to_ng`
  branch keyed on the digraph; AST asserts the generic `-k → -ğ` branch
  fires only when the preceding character is NOT `n`.
- [x] **Proof tests.** 12-row positive (5 catalog colours + 7 corpus
...
- [x] **The problem.** First/second-person/demonstrative pronouns have
...
- [x] **Fix.** Closed `ai/nlp/lang_tr/morph/pronouns_irregular.tr.yaml`
  ...
  `pronoun_marked=True`.
- [x] **Proof tests.** 48-row paradigm completeness (8 × 6); 20-row
...
- [x] **The problem.** Turkish often borrows English/Italian plurals as
...
- [x] **Fix.** Closed `ai/nlp/lang_tr/loanwords/loan_singularisation.tr.yaml`
...
- [x] **Proof tests.** 30-row positive + 10-row negative; hot-reload
  proof; `_xref` build refusal on ambiguous decision.

#### 10.29.6 Verbal-noun gerund vs nominalisation ambiguity

- [x] **The problem.** `-mak/-mek` (infinitive) vs `-ma/-me` (verbal
  ...
  - *"Galatasaray oynaması"* (missing genitive marker — common
    ...
    silently pick.
- [x] **Fix.** Closed `ai/nlp/lang_tr/morph/verbal_nouns.tr.yaml`
...
- [x] **The problem.** Turkish emphasises adjectives by full-word
  ...
  features.
- [x] **Fix.** New §10.1 step 7c `collapse_reduplication` AFTER 7a
...
- [x] **Proof tests.** 25-row positive + 15-row negative (`ev ev` →
...
- [x] **The problem.** *"Galatasaray ya da Fenerbahçe kim kazanır?"* is a
...
- [x] **Fix.** Closed `ai/nlp/lang_tr/morph/coordinating_particles.tr.yaml`
...
- [x] **The problem.** Turkish football fans write telegraphically:
  *"Galatasaray bugün"* — no `?`, no verb, no `mı`. §10.4 abstention
  ...
  entity + nothing else).
- [x] **Fix.** New deterministic heuristic in §10.4 BEFORE classifier:
...
- [x] **AST guard.** `test_nlp_telegraphic_inference_never_routes_predict`
...
- [x] **Proof tests.** 45-row corpus (3-token telegraphic / 4-token
...
- [x] **The problem.** §10.22.4 detached particle `ki` but treats every
...
- [x] **Fix.** Particle detacher gains context test: presence of comma
...
- [x] **Proof tests.** 25-row positive (relative + emphatic) + 10-row
...
- [x] **The problem.** Phase 7 Go sec layer (`server/internal/sec/sanitize.go`)
...
- [x] **Spec versioning.** `tr_normalize_spec.json` carries
...
- [x] **Proof tests.** Round-trip property (`normalize(normalize(x)) ==
...
- [x] **The problem.** §10.21.8 envelope HMAC catches forged
...
- [x] **Fix.** §5 specifies `prediction_id = sha256(match_id | market |
...
- [x] **Proof tests.** Synthesise an envelope with valid HMAC but
...
- [x] **The problem.** Cache key includes `pipeline_version` (§10.23.9).
...
- [x] **Fix.** Boot probe writes `cache:nlp:pipeline_max_seen` (Redis
...
- [x] **L0 cache invariant.** L0 entry carries the writer pod's
...
- [x] **Proof tests.** Boot-refuse test on cache-newer-than-self;
...
- [x] **Phase 4 (R1 storage).** No schema impact. Audit walks of
...
- [x] **Phase 5 (consensus).** No schema impact. New consumer test
...
- [x] **Phase 7 (sec).** Cross-language byte-parity gate per §10.29.11
  ...
  morphology helpers.
- [x] **Phase 8 (patcher).** Patcher EXCLUDES new tables under
  ...
  files).
- [x] **Phase 9 (gateway).** No new HTTP-surface change. Gateway
...
- [x] **Phase 11 (compute).** Telegraphic-inference helper, geminate /
  vowel-drop / pronoun helpers all CPU-only — AST guard rejects
  CUDA/MPS imports under `ai/nlp/lang_tr/`.
- [x] **Phase 12 (chaos).** New stubs: `chaos.tr-normalize-spec-drift`
  ...
  latency unchanged).
- [x] **Phase 13a (LeagueCatalog).** Build refusal extends to include
...
- [x] **~16 new cfg knobs:** `nlp_telegraphic_max_tokens=4`,
...
- [x] **8 new `nlp.event.v1` kinds** (open enum per §8.16.2):
...
- [x] **5 new `nlp.alert.v1` kinds** (debounced per §10.21.13 doctrine):
...
- [x] **~50 new proof tests** on top of cumulative §10.0–§10.28
  ≈ 535+ → cumulative Phase 10 ≈ 585+ tests.
- [x] **`make swarm.demo.nlp.full`** extends ≤ 60 s budget covering 6
  §10.29 paths (geminate, vowel-drop, pronoun, telegraphic,
  comparative, byte-parity).

#### 10.29.16 Definition of Done additions

- [x] All `[ ]` items in §10.29.1–§10.29.15 ticked.
- [x] §10.20 DoD item 27 added (this section's binding).
- [x] CODEOWNERS updated: `ai/nlp/lang_tr/spelling/{geminate_restoration,
...
- [x] **Versioning.** `swarm` patch (no public surface change beyond
