...
- [x] **Drift telemetry.** Per-week cue-fire-rate is exported as
...
- [x] **Proof:** `test_sarcasm_marker_table_codeowners_enforced`,
...
- [x] **Closed `question_tag_classifier.tr.yaml`** distinguishes
...
- [x] **Confirmation-seeking dispatch contract.** When
  ...
  - **Mismatch (presupposition false)** → closed prepend
    *"Aslında hayır, [X]."* with the corrected fact. Defends
    ...
    belief by omission.
- [x] **Rhetorical-negative disambiguation hook to §10.31.7.**
...
- [x] **Schema-version bump.** `qa.intent.v1` →
...
- [x] **Tier-blind invariant.** Pragmatic class never affects
...
- [x] **Proof:** `test_question_tag_classifier_distinguishes_three_shapes`,
...
- [x] **Closed `pro_drop_intent_classes.tr.yaml`** lists intent
...
- [x] **Pro-drop detector.** Runs after §10.5 entity extraction,
  ...
  - **Step 3 — disambiguation** (per `pro_drop_resolution_strategy=*_disambiguation`).
    ...
    *"Hangi takımı/oyuncuyu kastettiğinizi belirtir misiniz?"*
    Never silently pick.
- [x] **Pro-drop event audit.** Every pro-drop resolution emits
...
- [x] **AST guard: pro-drop only fires on grammatical pro-drop
...
- [x] **Cache-key salt.** L0 / L1 cache keys for pro-drop-resolved
...
- [x] **Proof:** `test_pro_drop_intent_class_table_complete`,
...
- [x] **Closed `possessive_chain_grammar.tr.yaml`** declares the
...
- [x] **Possessive-chain extractor.** Runs after §10.5 CRF
...
- [x] **Ambiguous-frame disambiguation.** When the chain admits
...
- [x] **Chain-depth cap.** `nlp_possessive_chain_max_depth=4`
...
- [x] **Cross-language byte parity.** `possessive_chain_grammar.tr.yaml`
...
- [x] **Proof:** `test_possessive_chain_grammar_parses_2_to_4_levels`,
...
- [x] **Closed `comparative_degree.tr.yaml`** lists Turkish
...
- [x] **Composition with §10.29.8 comparative fan-out and
...
- [x] **Negative-comparative semantic firewall.** Surface
...
- [x] **Proof:** `test_comparative_degree_table_complete`,
...
- [x] **Closed `score_notation_conventions.yaml`** declares the
...
- [x] **Disambiguation rule.** When (a) `score₁ == score₂` (draw)
...
- [x] **Score-as-input vs score-as-output discipline.** This
...
- [x] **Proof:** `test_score_notation_conventions_table_complete`,
...
- [x] **Closed `negation_scope_disambiguators.tr.yaml`** lists
...
- [x] **Resolution algorithm.** Compute a rhetoric_score from
...
- [x] **Composition with §10.31.2.** This section's resolution
...
- [x] **Drift telemetry.** Per-week distribution of rhetoric_score
...
- [x] **Proof:** `test_negation_scope_signal_table_complete`,
...
- [x] **Closed `refusal_template_pool.tr.yaml`** — every meta.*
  ...
  per §10.27.3).
- [x] **Closed `refusal_reason_code` taxonomy** (additive enum
...
- [x] **Analytics integration.** `qa.answer.v1.refusal_reason_code`
...
- [x] **Tier-blind invariant.** AST guard
...
- [x] **Proof:** `test_refusal_template_pool_min_size_per_reason`,
...
- [x] **Conflict-resolution invariant.** When `lexicon.tr.yaml`
...
- [x] **Boot-time conflict-detection scan.** `make verify.lexicon-catalog-consistency`
...
- [x] **Resolution-time tracing.** Every entity resolution that
  *could* have hit a conflict (the surface had both a lexicon
  ...
  goes unnoticed.
- [x] **Forward hook to Phase 13 release.** When a Phase 13
...
- [x] **Proof:** `test_lexicon_catalog_conflict_resolves_to_catalog`,
...
- [x] **`qa.answer.v1.outbound_checksum`** — additive field
...
- [x] **Gateway verification gate.** Phase 9 gateway, on
  ...
  on either end).
- [x] **Outbound-secret rotation.** `outbound_secret_per_pod`
...
- [x] **AST guard.** `test_outbound_checksum_field_set_on_every_qa_answer_v1`
...
- [x] **Proof:** `test_outbound_checksum_set_on_every_emission`,
...
- [x] **Closed `intent_slo_classes.yaml`** assigns every
...
- [x] **Per-class SLO measurement.** Prometheus histogram
...
- [x] **Class-vs-budget invariant.** `test_intent_slo_classes_budgets_strictly_ascending`
  asserts the three class budgets are strictly ascending so
  the multi-class burn-rate-alert routing has no overlap
  ambiguity.
- [x] **Cross-phase impact: Phase 9 gateway.** Gateway carries
...
- [x] **Proof:** `test_intent_slo_classes_table_covers_all_intent_ids`,
...
- [x] **Replace process-alive healthz with realism healthz.** The
  ...
  3. Compares actual `(intent_id, top_1_entity_id,
     ...
     reason; match → 200.
- [x] **Performance budget.** Probe cost ≤
...
- [x] **Probe rate-limit & memoisation.** Probe runs at most
...
- [x] **Golden refresh discipline.** `healthz_golden.json` is in
  ...
  any template change.
- [x] **AST guard.** `test_healthz_realism_probe_runs_full_pipeline_ast`
...
- [x] **Proof:** `test_healthz_realism_returns_200_on_match`,
...
- [x] **Shared-budget invariant.** When §10.24.5 splits a run-on
  ...
  class.
- [x] **Subquery-fan-out cap.** Per-request cap
  ...
  *"Sorgunuzu daha kısa cümlelere bölerseniz daha iyi
  yanıt verebilirim."*
- [x] **Single-subquery degenerate case invariant.** When the
...
- [x] **Cross-cutting refusal vs partial-success contract.**
...
- [x] **Proof:** `test_multi_subquery_budgets_divided_by_share_factor`,
...
- [x] **Canary shadow-comparison protocol.** Promoting a new
...
- [x] **Shadow-comparison time budget.** Comparison runs in the
...
- [x] **Comparison-corpus sampling discipline.** Sampled
...
- [x] **Forensic trace.** Every disagreement row recorded as
...
- [x] **Composition with §10.25.6 governance.** Canary
...
- [x] **Proof:** `test_lexicon_canary_runs_in_shadow_only`,
...
- [x] **Configuration knobs (~25 new keys, §10.19 triangle update).**
  | Key | Default | Purpose |
  |---|---|---|
...
- [x] **CODEOWNERS additions.** New files in the §10.30.15 group:
...
- [x] **Versioning.** `swarm` minor (additive
