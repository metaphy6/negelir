...
- **Phase 4.6 telemetry watch set.** `telemetry.v1._WATCHED_TOPICS` extends to include `QA_REQUEST_V1`, `SEC_ALERT_V1`, `SEC_QUARANTINE_V1`, `SEC_DENYLIST_V1`. Cardinality stays bounded by the topic enum (per the N3 fix and the C-level cardinality note in the pre-Phase-7 audit §1.4).
...
- [ ] **Topology.** Two surfaces, one boundary:
...
- [ ] **Detects:** prompt injection ("ignore previous instructions", system-prompt smuggling, role-play jailbreaks), tool-call smuggling, base64-encoded payloads with `eval`/`exec`/`__import__` markers, zero-width chars, RTL overrides, oversized inputs (`> cfg.sec_input_max_len`, default 8 KiB), language-spoofing (claims TR but body is JS / shell / SQL exfil), homoglyph attacks (Cyrillic 'е' for Latin 'e' in known team names — uses Phase 13 lexicon when available; deterministic NFKC fold in v1).
- [ ] **Detection strategy.** Fast **deterministic** rules first (regex over a YAML pattern file at `ai/common/security/injection_patterns.yaml`, charset bitmap, length, NFC + NFKC normalization, Turkish-dotted-i rules); only escalate ambiguous cases to a small classifier (`distilbert-base-multilingual-cased`-style, ≤ 60 MB on disk, ≤ 200 MB resident) running on GPU when available, CPU fallback per Phase 11.3.
- [ ] **Verdicts:** `pass`, `sanitize`, `quarantine`. Quarantined → published to `sec.quarantine.v1` (envelope: `{quarantine_id, source=qa|scrape, raw_b64 (capped at cfg.sec_quarantine_payload_max_bytes), verdict=quarantine, reasons[], detected_at, client_id?, ip?}`) and persisted by Phase 4 `storage.v1` to the `quarantine_samples` table (Phase 9 migration `007_quarantine.sql`). TTL configurable (`cfg.sec_quarantine_ttl_days`, default 30); a Phase 8 `maint.backup.v1` job prunes. Quarantined samples **never** reach predictors and **never** appear on `qa.request.v1`.
- [ ] **Latency budget.** Gateway middleware: hard cap `cfg.sec_input_gateway_max_latency_ms` (default 10 ms). Agent classifier: hard cap `cfg.sec_input_classifier_max_latency_ms` (default 100 ms). On breach → circuit breaker opens for `cfg.sec_input_breaker_open_s` (default 30 s); verdict defaults to `pass` (fail-open) and `sec.alert.v1{kind=classifier_degraded, severity=warn}` is emitted exactly once per breaker-open window (debounced).
- [ ] **No `proof.flag` emission.** All sec.input verdicts go to `sec.alert.v1` / `sec.quarantine.v1`; the `proof.flag` topic stays scoped to post-prediction sanity (Phase 6 doctrine).

### 7.2 Upstream-data anomaly agent (`sec.scrape.v1`)

- [ ] Subscribes to `scrape.raw` filtered to **HTTP 200 + non-empty body**. Failures (404 / 5xx / empty body) stay on the Phase 4.1 path (`proof.flag` / DLQ); boundary test enforces no double-handling.
- [ ] Watches for: sudden body-size delta (`> cfg.sec_scrape_size_delta_pct`, default 200%, vs. rolling source baseline), suspicious JS markers in DOM bodies (regex over a configurable allowlist of known-good script srcs from the seed corpus), encoded redirects (`<meta http-equiv="refresh">`, JS `location` writes inside the first 4 KiB), mismatched content-type (header vs. magic-byte sniff), inflate-ratio outliers (gzip / brotli expansion > `cfg.sec_scrape_inflate_ratio_max`, default 50× — defense against zip-bomb-style upstream payloads).
- [ ] **Baseline storage.** In dev / CI: compares `bytes_sha256` against [`infra/mock/seeds/manifest.json`](../../infra/mock/seeds/manifest.json) (exact match expected; mismatch is a hard `sec.alert.v1{severity=critical}`). In prod: rolling-30-day fingerprint distribution per source, persisted in Postgres (`source_fingerprints` table, Phase 9 migration `008_source_fingerprints.sql`); statistic = (median body size, IQR, p95 inflate ratio, content-type histogram). Memory bound: in-process LRU per source capped at `cfg.sec_scrape_baseline_max_per_source` (default 5000 entries, ~4 MiB per source at typical sizes).
- [ ] **Verdict routing.** Bytes-on-the-wire anomalies (encoded redirect, mismatched content-type, inflate-ratio outlier, suspicious-JS marker) → `sec.alert.v1`. Post-parse-shape anomalies that surface upstream of categorizer (e.g., DOM drift detected by structural fingerprint divergence) → `proof.flag{kind=parse_failed}` so the Phase 17 patcher can pick them up. Routing is a single-decision function with a unit test per branch.
- [ ] **Bounded state.** Rolling baseline is `cfg.sec_scrape_baseline_max_per_source`-bounded; pending classification queue is `cfg.sec_scrape_max_pending`-bounded with insertion-order LRU eviction (mirrors the Phase 6 `drift.v1` `_pending` pattern).

### 7.3 Abuse / rate-limit agent (`sec.rate.v1`)

- [ ] **Two-tier enforcement.**
  - **Pre-auth (per IP):** lower limits, defense against credential stuffing on `/v1/auth/login`. Bucket capacity `cfg.sec_rate_pre_auth_capacity` (default 30 tokens), refill `cfg.sec_rate_pre_auth_refill_per_s` (default 0.5/s).
...
- [ ] **Implementation.** Token-bucket math runs **inside Redis** via an atomic Lua script (`infra/redis/lua/sec_rate_gca.lua`, GCRA — Generic Cell Rate Algorithm). Atomic; no read-modify-write race across replicas. Bucket state TTL = `cfg.sec_rate_bucket_idle_ttl_s` (default 3600); idle buckets evict naturally and do not pin Redis memory. The Go gateway calls the Lua script on every request (one round-trip; no Python-agent hop on the hot path); the `sec.rate.v1` Python agent runs only the **anomaly detector** (sliding-window burst detection) and the **alert + denylist publisher**.
- [ ] **Sliding-window burst detection** (`cfg.sec_burst_threshold`, e.g. > 100 distinct rejected attempts in `cfg.sec_burst_window_ms` from a single subject). Window math uses `time.monotonic()`-based ms (per pre-Phase-7 audit M2/M4); on-the-wire `produced_at` stays wall-clock.
- [ ] **Denylist.** Sole writer of the Redis hash `sec:denylist:{client_id|ip}` with TTL `cfg.sec_denylist_ttl_s` (default 3600 = 1 h, escalating per offense via `cfg.sec_denylist_escalation_factor`). Consulted by the Go gateway on every request **before** the bucket check (one Redis call; co-located with the bucket Lua script in a `MULTI` to keep latency at one round-trip total). Denylist mutations also publish `sec.denylist.v1{action=add|remove, subject, ttl_s, reason, decided_at}` so dashboards stay in sync without polling Redis.
- [ ] **Operator override hook.** Subscribes to `maint.event.v1{kind=denylist_clear, target=<subject>}` (Phase 8 publisher) and removes the matching denylist entry, emitting a `sec.denylist.v1{action=remove, reason=manual_override}` event.
- [ ] **Bounded subject map.** In-process LRU of subjects-being-watched capped at `cfg.sec_rate_max_subjects` (default 100 000, mirrors `cfg.drift_max_settled`). Eviction is silent (memory bound, not correctness signal — the Redis bucket survives eviction).
- [ ] **Single-instance.** `sec.rate.v1` joins `SINGLE_INSTANCE_AGENTS` (per pre-Phase-7 audit M1's frozenset contract) because it is the sole writer of the denylist hash and the sole publisher of `sec.denylist.v1`. Multi-replica coordination needs Postgres-backed leader election (Phase 14.x follow-up).

### 7.4 `sec.alert.v1` envelope and severity ladder

- [ ] **Schema** (`ai/swarm/sdk/schemas/sec.alert.v1.json`). Fields: `{alert_id, kind ∈ SecAlertKind, severity ∈ {info, warn, error, critical}, source ∈ {sec.input.v1, sec.scrape.v1, sec.rate.v1}, subject?, request_id?, client_id?, ip?, evidence_ref?, reason, produced_at}`. `evidence_ref` is a pointer (sha256 + blob URI) into `sec.quarantine.v1` payloads — the alert never inlines the offending bytes (cardinality + log-injection guard).
- [ ] **`SecAlertKind` enum** (extensible, additions are minor bumps): `prompt_injection`, `payload_oversize`, `charset_anomaly`, `language_spoof`, `homoglyph_attack`, `classifier_degraded`, `dom_size_delta`, `suspicious_js`, `encoded_redirect`, `content_type_mismatch`, `inflate_ratio_outlier`, `seed_drift`, `rate_burst`, `denylist_added`, `denylist_removed`, `denylist_growth_anomaly`.
- [ ] **Severity ladder.** `info`: observable, no action (e.g. `denylist_added` for a known-bad IP). `warn`: human review during business hours (e.g. `classifier_degraded`). `error`: alert on-call (e.g. `dom_size_delta` from a real upstream). `critical`: page (e.g. `seed_drift` in CI — supply-chain signal). The Phase 4.6 telemetry agent counts alerts by `(kind, severity)` and surfaces them on the Prometheus page; cardinality is bounded by the enum.

### 7.5 Topic catalog additions

To be added to Phase 3 §3.5 in lock-step with this phase:

| Topic | Producer | Consumer(s) | Payload |
|---|---|---|---|
| `qa.request` | API gateway (raw, post-deterministic-rules) | `sec.input.v1` | `{request_id, raw_text, client_id?, ip, locale, received_at}` (control-plane, unversioned) |
| `qa.request.v1` | API gateway (verdict=pass) **or** `sec.input.v1` (verdict=pass post-classifier) — exactly one producer per request, never both | NLP layer (Phase 10) | `{request_id, sanitized_text, client_id?, locale, sec_verdict ∈ {pass, sanitized}, sec_steps_run[], emitted_at}` |
| `sec.alert.v1` | `sec.input.v1`, `sec.scrape.v1`, `sec.rate.v1` (open producer set within `sec.*` only) | telemetry, supervisor, ops dashboards | per §7.4 |
| `sec.quarantine.v1` | `sec.input.v1` (QA path), `sec.scrape.v1` (scrape path) | `storage.v1` (persists to `quarantine_samples`) | per §7.1 |
| `sec.denylist.v1` | `sec.rate.v1` (SOLE producer) | API gateway cache, telemetry | per §7.3 |
...
- [ ] `qa.request.v1` has at most two producers (`server.gateway` + `sec.input.v1`) and exactly one consumer (NLP, Phase 10 placeholder until then).
- [ ] `sec.denylist.v1` has exactly one producer (`sec.rate.v1`).
- [ ] `sec.quarantine.v1` consumer set is `{storage.v1}` only (no predictor / NLP / proofreader subscriber).
- [ ] `sec.input.v1` and `sec.scrape.v1` **do not** publish `proof.flag` (the two taxonomies are disjoint per §7 forward contracts).
- [ ] `sec.scrape.v1` does **not** consume `proof.flag` (no rebound on Phase 4.1 failures).
- [ ] NLP layer (Phase 10) does **not** subscribe to `qa.request` (raw).
- [ ] `sec.rate.v1` is the only writer of the Redis `sec:denylist:*` keys (Lua-script audit + integration test against `RedisStreamsBus` profile).
...
- [ ] Three default sec agents (`sec.input.v1`, `sec.scrape.v1`, `sec.rate.v1`) implemented + voting against `InMemoryBus` through `AgentRunner`. Each lives under `ai/swarm/agents/sec/` (mirrors the Phase 6 `proofreader/` package layout).
- [ ] Go gateway middleware (`server/internal/sec/input.go`, `server/internal/sec/rate.go`) implements the in-process tier; integration test against the gateway + Redis confirms median QA latency stays under `cfg.sec_input_gateway_max_latency_ms` for benign payloads.
- [ ] **Fail-open doctrine.** No defense agent ever blocks the **happy path** in CI's golden-input suite (200+ benign Turkish queries from [`ai/tests/fixtures/turkish_queries.yaml`](../../ai/tests/fixtures/turkish_queries.yaml), Phase 10) — explicit negative test: kill `sec.input.v1`, run the golden suite, expect 0 quarantines and ≤ N `classifier_degraded` warns where N = ceil(suite_size / cfg.sec_input_breaker_open_s).
- [ ] **Idempotency.** Each sec agent's handler is idempotent under at-least-once redelivery: re-publishing the same `request_id` does not double-add to the denylist, double-quarantine, or double-emit `sec.alert.v1` (regression tests mirror the Phase 6 `test_phase6_bug_fixes.py` style).
- [ ] **Bounded state.** Every in-process map is config-capped with insertion-order LRU eviction (mirror Phase 5 `consensus.v1` and Phase 6 `drift.v1` patterns). Overflow eviction emits `sec.alert.v1{kind=…, severity=warn}` outside the lock.
- [ ] **Schema parity.** Every Phase 7 message validates against its JSON Schema at the live emission point (`test_schemas_match_payloads.py` extension + live-emission probes in a new `test_phase7_bug_fixes.py`).
- [ ] **Boundary tests** (§7.6) all green.
- [ ] **swarmctl visibility.** Every Phase 7 agent appears in `swarmctl ps`; every Phase 7 topic appears in `swarmctl topics`.
- [ ] **Telemetry watch set** extended (per cross-phase alignment above); regression test mirrors `test_telemetry_watches_phase6_proofreader_drift_topics`.
- [ ] **Phase 12 catalogue stubs.** Every check listed in §7.1 / §7.2 / §7.3 has a corresponding *stub* row in `docs/testing/phase12_catalogue.md` (one row per check, body = "TBD — implementation pending Phase 12"). The stubs satisfy the original Phase 7.4 DoD bullet without requiring Phase 12 to land first; the bullet flips to fully green when Phase 12 fills the bodies in.
- [ ] **Config triangle green.** All Phase 7 knobs surface in `xops/env/.env.example`, `ai/common/defaults.yaml`, and `ai/common/config.py` — `test_config_sync` stays green. Knob inventory: `sec_input_max_len`, `sec_input_gateway_max_latency_ms`, `sec_input_classifier_max_latency_ms`, `sec_input_classifier_device`, `sec_input_classifier_path`, `sec_input_breaker_open_s`, `sec_quarantine_ttl_days`, `sec_quarantine_payload_max_bytes`, `sec_scrape_size_delta_pct`, `sec_scrape_inflate_ratio_max`, `sec_scrape_baseline_max_per_source`, `sec_scrape_max_pending`, `sec_rate_pre_auth_capacity`, `sec_rate_pre_auth_refill_per_s`, `sec_rate_post_auth_capacity`, `sec_rate_post_auth_refill_per_s`, `sec_rate_bucket_idle_ttl_s`, `sec_rate_max_subjects`, `sec_burst_threshold`, `sec_burst_window_ms`, `sec_denylist_ttl_s`, `sec_denylist_escalation_factor`.
- [ ] **Versioning.** `swarm` minor bump at the Phase 7 landing; `server` minor bump for the Go gateway middleware; subsequent Phase 7 review patches bump `swarm` / `server` patch-level.
- [ ] **Migrations.** `migrations/007_quarantine.sql` (the `quarantine_samples` table) and `migrations/008_source_fingerprints.sql` (rolling-30-day baseline) land with this phase; both are additive (no `DROP`).
