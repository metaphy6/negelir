...
- [ ] **Auth-endpoint scope.** `sec.input.v1` runs on every QA payload AND on `/v1/auth/login` / `/v1/auth/register` request bodies (the `email` / `password` fields are user-controlled and just as injection-prone as QA text). The `password` field is excluded from quarantine payloads regardless of verdict (PII redaction at the source) — `pii_redacted=true` on the `sec.quarantine.v1` envelope.
...
- [ ] **Config triangle green.** All Phase 7 knobs surface in `xops/env/.env.example`, `ai/common/defaults.yaml`, and `ai/common/config.py` — `test_config_sync` stays green. Knob inventory: `sec_input_max_len`, `sec_input_gateway_max_latency_ms`, `sec_input_classifier_max_latency_ms`, `sec_input_classifier_device`, `sec_input_classifier_path`, `sec_input_breaker_open_s`, `sec_input_pattern_reload_s`, `sec_quarantine_ttl_days`, `sec_quarantine_payload_max_bytes`, `sec_scrape_size_delta_pct`, `sec_scrape_inflate_ratio_max`, `sec_scrape_warmup_samples`, `sec_scrape_baseline_flush_s`, `sec_scrape_max_pending`, `sec_rate_pre_auth_capacity`, `sec_rate_pre_auth_refill_per_s`, `sec_rate_post_auth_capacity`, `sec_rate_post_auth_refill_per_s`, `sec_rate_bucket_idle_ttl_s`, `sec_rate_max_subjects`, `sec_rate_ipv4_prefix`, `sec_rate_ipv6_prefix`, `sec_rate_trusted_proxies`, `sec_rate_redis_timeout_ms`, `sec_rate_secondary_capacity`, `sec_rate_secondary_refill_per_s`, `sec_burst_threshold`, `sec_burst_window_ms`, `sec_denylist_ttl_s`, `sec_denylist_escalation_factor`, `sec_denylist_max_entries`, `sec_alert_debounce_ttl_s`, `sec_alert_critical_debounce_enabled`. (The earlier `sec_scrape_baseline_max_per_source` knob is retired — the streaming-statistic baseline replaces the capped LRU; absence of the knob is asserted in `test_config_sync` so it cannot creep back.)
