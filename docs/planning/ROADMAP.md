...
| 3 — Swarm foundation | ✅ done | SDK at `swarm/sdk/`; all 7 §3.7 DoD tests green; NATS + bus auth deferred. |
...
|---|---|---|
| 1 | **🔧 Single-Source Configuration** | Every tunable value comes from one of: `common/config.py` (Python) or `internal/config` (Go). Both read env vars with documented defaults from `.env.example`. **No magic numbers in code, ever.** |
...
- [x] In `orchestrator/state_machine.py`: remove `--mode sim-only` / `--skip-p2p` paths and the P2P stage from the full pipeline.
- [x] `grep -RIn "p2p\|P2P\|peer\|gossip\|multicast" ai/ server/ --include='*.py' --include='*.go'` must return **0 lines** outside test fixtures.
      *(Verified 2026-04-19. Only matches: `qid/collector.py` reuses `peer` for swarm peers; `test_config_sync.py::test_p2p_module_removed` is the regression guard; `training_pipeline.py` has a docstring noting the removal.)*
...
- [x] Docstrings corrected in `common/config.py`, `scraper/real_data.py`, `model/real_features.py`, `scraper/mackolik.py` (the last is now correctly labelled "TR-source adapter for the seeded default league").
- [x] `common/betting_markets.json` — catalogue audit (2026-04-19):
...
### 1.1 Python config (`common/config.py`)
...
- [x] `swarm/__init__.py` + `swarm/source_watcher/__init__.py` — package home.
...
- [x] Integration test: snapshot store + planner against a frozen pair of seed files (`swarm/source_watcher/tests/fixtures/openfootball_v{1,2_breaking}.json` + `test_fixture_integration.py`).
...
**Depends on:** Phase 1
**Pivot v3 placement:** the agent SDK is shared infrastructure consumed by both `swarm/` (AI agents) and `datasource/` (scraper/watcher/refresher/patcher). After Phase R2 it lives at **`common/bus/`**. Until R2 lands, this phase ships under the transitional path **`swarm/sdk/`** with the namespace structured so R2 is a directory move, not a rewrite.
...
`swarm/sdk/` (Python). The package layout is the post-R2 shape, importable as `swarm.sdk.*`-style internally so the R2 move is `git mv swarm/sdk common/bus`:
...
> **Wire authority.** The dataclasses in `swarm/agents/payloads.py` plus the matching JSON Schemas in `swarm/sdk/schemas/<topic>.json` are the **binding** wire contract; the table below is a human-readable summary that may abbreviate optional fields. Any disagreement between the two is a bug — `swarm/agents/tests/test_schemas_match_payloads.py` keeps them honest.
...
- [x] All seven config knobs from §3.3 surface in `xops/env/.env.example`, `common/defaults.yaml`, and `common/config.py`. Triangle test stays green.
...
- **Phase 3 §3.5 topic catalog.** The wire authority for every Phase 4 topic. Payload changes go through `swarm/agents/payloads.py` + `swarm/sdk/schemas/<topic>.json`; the §3.5 table is a human summary that must be updated in lock-step.
...
- [x] `ReactorBase` at `swarm/agents/reactor.py` enforces
...
- [x] Trainer reactor with debounce window, qualifying-event filter,
      ...
      Publishes `model.trained` on `models.events.v1`. *(Shipped in Phase 5.5; see `swarm/agents/reactor.py::TrainerReactor`. Box flipped per [`reports/phase6-second-pass.md`](../reports/phase6-second-pass.md) F2-4 — duplicate of §5.5 entry.)*
...
- [x] All Phase 4 config knobs surface in `xops/env/.env.example`,
      `common/defaults.yaml`, and `common/config.py` (triangle
      test green).
...
- **Phase 3 §3.5 topic catalog (wire authority).** Phase 5 lands the JSON Schemas for `predict.request`, `predict.vote`, `predict.final` at `swarm/sdk/schemas/<topic>.json`, plus `PREDICT_VOTE` constant in `swarm/agents/topics.py` (today only `PREDICT_REQUEST`/`PREDICT_FINAL` exist). Payload dataclasses live in `swarm/agents/payloads.py` next to the Phase 4 ones; `test_schemas_match_payloads.py` is extended to cover them.
...
- [x] All §5.1 **must**-status predictors (`elo`, `dixon_coles`, `xgb_form`, `xgb_xg`) implemented + voting against `InMemoryBus` through `AgentRunner` in `swarm/agents/tests/test_phase5_swarm_demo.py` (the `make swarm.demo` driver covers Phase 4; Phase 5 is exercised via the same in-process runner stack rather than a separate make target so it runs in CI). `pred.lgbm_market.v1` ships behind a default-off flag; `pred.tabnet.v1` deferred per the §5.1 gate.
- [x] `consensus.v1` produces exactly one `predict.final` per `(match_id, market, request_id)` under at-least-once redelivery (idempotency test in `swarm/agents/tests/test_consensus.py`).
...
- **Phase 3 §3.5 topic catalog (wire authority).** Phase 6 lands JSON Schemas at `swarm/sdk/schemas/{predict.proofreader_verdict.v1,predict.approved.v1,match.outcome.v1,maint.event.v1}.json` plus the `MAINT_EVENT`/`MATCH_OUTCOME`/`PROOFREADER_VERDICT`/`PREDICT_APPROVED` constants in `topics.py`. Payload dataclasses live in `payloads.py`. `test_schemas_match_payloads.py` covers them.
...
- [x] Three default proofreader replicas (`proofreader.sanity.v1`, `proofreader.plausibility.v1`, `proofreader.consistency.v1`) implemented + voting against `InMemoryBus` through `AgentRunner` in [`test_proofreader_replicas.py`](../../swarm/agents/tests/test_proofreader_replicas.py). Each runs ONE check from `prediction_checks.py` (failure isolation; scale symmetry per Doctrine §2 Rule 5).
- [x] `proofreader_aggregator.v1` produces exactly one `predict.approved.v1` per `prediction_id` under at-least-once redelivery (idempotency test in [`test_proofreader_aggregator.py`](../../swarm/agents/tests/test_proofreader_aggregator.py); single-publication guarded by the Phase 4.7 `Ledger` Protocol).
...
- **Phase 3 §3.5 topic catalog.** Phase 7 introduces five new topics — `qa.request`, `qa.request.v1`, `sec.alert.v1`, `sec.quarantine.v1`, `sec.denylist.v1` (see §7.5). Each must land with (a) a row in §3.5, (b) a JSON Schema in `swarm/sdk/schemas/<topic>.json`, (c) a config-driven consumer-group prefix. The §3.5 `sec.alert` row is replaced with the versioned `sec.alert.v1` row (data-plane envelope, see §7.5 below).
...
- [x] **Topology.** Two surfaces, one boundary:
  ...
  - Python agent (`swarm/agents/sec/input.py`): subscribes to `qa.request`, runs the small classifier (step 5), publishes `qa.request.v1` (verdict `pass`) or `sec.quarantine.v1` (verdict `quarantine`), and emits `sec.alert.v1{severity=info|warn}` for every escalation that resolved to `quarantine`.
...
To be added to Phase 3 §3.5 in lock-step with this phase. Every row lands with (a) a JSON Schema in `swarm/sdk/schemas/<topic>.json`; (b) for control-plane (unversioned) topics, the schema carries the C2 marker `"comment": "control-plane: unversioned by design (ROADMAP §3.7)"` per the pre-Phase-7 audit cleanup.
...
`swarm/agents/tests/test_boundary_discipline.py` extensions (mirrors the Phase 6 §6.4 boundary tests):
...
- [x] `sec.rate.v1` is the only writer of the Redis `sec:denylist:*` keys (Lua-script audit + integration test against `RedisStreamsBus` profile). *(Closed: AST scan in `test_boundary_discipline.py::test_sec_denylist_v1_no_unauthorised_writer_in_registry` plus live-Redis integration in `swarm/agents/tests/test_phase7_lua_redis.py` exercising `sec_denylist_mutate.lua` add/remove/cap/refresh-not-stack semantics.)*
...
- **Phase 3 §3.5 topic catalog (wire authority).** Phase 8 lands **one** new topic — `maint.ack.v1` (per-consumer acknowledgements; `maint.event.v1` and `maint.dlq.v1` already exist). The §3.5 wire-authority table grows by exactly one row (`maint.ack.v1` → schema `swarm/sdk/schemas/maint.ack.v1.json`, producer set `{any consumer of maint.event.v1}`, consumer set `{ops_console, maint.* operator-replay paths}`). New `maint.event.v1.kind` values join the open-enum allow-list (§7.4 pattern):
...
> goes through `common/config.py` + `xops/env/.env.example` (Rule 1).
> The legacy `AI_DEVICE` env var (used by `model/device.py` today) is
...
| Source | File | Theme |
|---|---|---|
| §11.1 | [`01-device-probe-registry.md`](../design/phase11/sections/01-device-probe-registry.md) | Device probe & registry (`model/device.py` + `common/devices/`) |
| §11.2 | [`02-gpu-arbiter.md`](../design/phase11/sections/02-gpu-arbiter.md) | GPU arbiter (`swarm/sdk/gpu_arbiter.py`) |
...
| §11.13 | [`13-heterogeneous-concurrent-routing.md`](../design/phase11/sections/13-heterogeneous-concurrent-routing.md) | Heterogeneous concurrent routing (`swarm/sdk/compute_router.py`) |
...
| §11.20 | [`20-tests.md`](../design/phase11/sections/20-tests.md) | Tests (`ai/tests/test_compute_*.py`, `swarm/tests/test_gpu_arbiter.py`, `swarm/tests/test_cpu_governor.py`) |
...
> ℹ️ The seeded default since v2.0.0 is the **Turkish Süper Lig** (`tr_super_lig`). All other leagues land via the catalog file `common/league_catalog.yaml` + per-league preset modules under `common/leagues/<league_id>.py` (LEAGUE_CATALOG.md §6). **No new code path branches on `league_id` after this phase** — every cross-league behaviour is data in the catalog or a calibration profile.
...
**Goal:** Close the loop between “a scraper broke” and “a PR fixed it.” Ship the `patcher` + `gitops` components (at `patcher/` and `gitops/` in the repo root after Phase 22; Phase 17’s implementation bullets reference the transitional `patcher/` and `gitops/` paths — §22.3 renames them) with the guardrails specified in [`design/SCRAPER_PATCHER.md`](../design/SCRAPER_PATCHER.md), wired so that *every* code path that could write to the repo, spend money on an LLM, or relax a detector is gated, observable, reversible, and reproducible.
...
| 29 | "Bundle-builder can read raw bytes straight from the source URL." | Direct upstream reads at bundle-build time turn the patcher into a scraper of last resort and bypass the source registry / robots policy. Bundle builder ONLY reads from feeds (`feeds/ops/raw/<source>/<date>/<artifact_id>.bin`) populated by the detector that emitted the artifact. Lint forbids `requests.`/`httpx.`/`urllib.` under `patcher/`. | `test_bundle_builder_no_network_imports.py` (lint), `test_bundle_builder_reads_only_from_feeds.py` |
...
| 51 | "The patcher can patch itself." | A patcher that can edit its own gates is a defeated patcher. Any artifact whose scope would touch `patcher/**`, `gitops/**`, `xops/patcher/**`, `xops/lint/phase17_*`, `xops/lint/feeds_*`, or `common/schemas/patcher/**` is auto-escalated with `reason=self_modification_forbidden`; no AI session opens, no diff produced, the artifact is paged to a human within `cfg.patcher_self_modification_page_sla_s` (default 60). | `test_patcher_cannot_patch_itself.py`, `test_self_modification_artifact_paged_to_human.py`, `test_self_modification_emits_zero_api_calls.py` |
...
- [ ] **Forbidden-pattern list externalised** to `xops/patcher/forbidden_patterns.yaml` (ledger #33); lint asserts the patcher imports the YAML; every row has `{regex, kind, severity, ledger_ref}` and at least one negative-case proof test under `patcher/tests/forbidden/`.
- [ ] **PII redaction** (ledger #12). `patcher/pii.py` covers TR ID number, IBAN, email, TR phone (`+90...`), street address heuristic; runs over every byte that goes to the API; produces `<REDACTED:KIND>` markers; counts `patcher_pii_redactions_total{kind}`; the local sandbox copy is the **un**redacted bytes (so the gauntlet can reproduce the failure).
...
- [ ] **Cost prediction pre-flight** (ledger #89). `patcher/cost_predict.py` refuses sessions whose predicted spend × `cfg.patcher_cost_predict_safety_factor` (default 1.5) exceeds `_PER_ARTIFACT_USD_CAP`; refused artifacts park (no escalation).
...
- [ ] **Audit trail.** Every config-flag read for a critical env var goes through `patcher/audit.py` which compares current value against the last-seen value in `postgres.patcher_config_audit`; mismatch with no audit row ⇒ `patcher.config.unaudited.v1` event + pause (ledger #34).
...
- [ ] **Per-file advisory lock** (ledger #37). `patcher/locks.py` acquires Redis `SET patcher:file:<sha256(path)> <session_id> NX PX <ttl_ms>` for every path the bundle's scope touches; failed acquisition parks the artifact and emits `patcher.file_locked.v1`. Locks released on session end (success / abort / supervisor crash via TTL).
...
- [ ] **UTC-only timestamps** (ledger #68). Ingress validator at `patcher/ingress.py` rejects naive datetimes; quiet-hours is the only sanctioned local-time conversion.
...
- [ ] **Past-fix BM25 seeding** (ledger #43). `patcher/seeding.py` ranks past examples by BM25 over the diagnostic excerpt; per-scope LRU bounded by `cfg.patcher_seeded_examples_max_per_scope` (default 8); evicted examples archived to `feeds/ops/bundles/.archive/seeds/`.
...
- [ ] `gitops/` scaffolded: GitHub App token exchange, branch + PR open, polling loop, label handling, post-merge watchdog skeleton (active in §17.6).
...
- [ ] **Affected-consumers labelling** (ledger #49). `gitops/import_graph.py` walks the static import graph at PR-open time; labels `affects:swarm`, `affects:server`; comment lists the touched extractor symbols and downstream consumers; missing `affects:*` when consumers exist ⇒ CI fails.
...
- [ ] **Cool-down clock is Postgres `now()`** (ledger #77). `gitops/cooldown.py` consults Postgres exclusively; lint forbids local-clock reads in the cool-down module; replicas with NTP drift > `cfg.patcher_clock_skew_alert_ms` (default 500) refuse to make merge / revert decisions and emit `sec.alert.v1{kind=patcher_clock_skew}`.
...
- [ ] **Wilson-interval parity** (ledger #57). `gitops/parity_stats.py` computes the Wilson 95 % lower confidence bound vs trailing-7 d baseline; revert fires only when the lower bound regresses past `cfg.gitops_regression_threshold` (default 5 %); window minimum `cfg.gitops_regression_min_samples` (default 30) — below this, watchdog defers and emits `gitops.regression.deferred.v1`.
...
- [ ] **Cross-source artifact coalescing** (ledger #67). `patcher/correlation.py` groups artifacts within a `cfg.patcher_correlated_window_s` window sharing a structural fingerprint (`sha256(kind || error_template || stack_root)`); ≥ `cfg.patcher_correlated_artifact_threshold` (default 3) coalesces into a parent artifact; one session, one diff, fanned out across the affected scope-paths.
...
- [ ] `patcher/tests/test_closed_loop.py` — a deliberately tampered seed triggers an artifact → diagnostic bundle → patcher session → diff → 5-gate gauntlet → bundle hand-off → `gitops` PR open → CI green (mocked) → (fake-clock cool-down elapses) → 5-gate evaluation → auto-merge → squash trailer parsed → tracker row written.
...
**Phase 18 conformance (binding):** Every PR that touches `common/league_catalog.yaml`, `common/leagues/`, `infra/mock/seeds/`, or `common/isolation/policy.yaml` must pass `xops/lint/phase18_conformance.py` in CI. This is enforced by §19.1 and verified by §19.12.
...
- [x] **Phase 17 watchdog coordination gate.** If the patcher watchdog (`swarm/agents/patcher/watchdog.py`) is active, Phase 19 verifies the watchdog's per-league artifact inventory references no frozen `ai/` path before a new T3 league is onboarded. This check runs as part of `make league.onboard` (§19.3). `test_19_1_watchdog_inventory_no_frozen_path.py`.
- [x] **Phase 18 conformance wired into all onboarding PRs.** `xops/lint/phase18_conformance.py` runs in CI on every PR touching `common/league_catalog.yaml`, `common/leagues/`, `infra/mock/seeds/<source>/`, or `common/isolation/import_graph.snapshot.json`. Failure blocks merge — no override. `test_19_1_phase18_conformance_blocks_bad_onboarding_pr.py`.
...
- [x] **Bulk gazetteer loader** (`nlp/gazetteer_bulk_loader.py`). Accepts a league's `LeagueConfig.team_name_map` and auto-generates: Turkish transliterations from `nlp/transliteration_rules.yaml`, phonetic aliases, short-form aliases. The loader is fully deterministic and idempotent — same input always produces identical output, no stochastic path. `test_19_4_bulk_loader_deterministic.py`.
- [x] **Transliteration rule coverage for all catalog scripts.** `nlp/transliteration_rules.yaml` must have coverage for every script used by any league in `league_catalog.yaml`. CI asserts: for each league row, every team name's Unicode script is covered by at least one transliteration rule. `test_19_4_transliteration_covers_all_scripts.py`.
...
- [x] **Phonetic matching for non-Latin scripts.** For scripts where direct transliteration produces poor phonetic fidelity (Arabic, Korean Hangul, Chinese Hanzi), `nlp/transliteration_rules.yaml` includes a `phonetic_weight: high` flag that activates the fuzzy-phonetic pass in the entity matcher. The phonetic pass is a separate rule layer (not the same edit-distance pass used for Latin-script typos); it is disabled for Latin-script leagues to avoid false positives. `test_19_4_phonetic_pass_enabled_for_arabic.py`, `test_19_4_phonetic_pass_disabled_for_latin.py`.
...
- [x] **Per-league red-team corpus** (inherits Phase 13 §13.28). Each newly onboarded T3 league must add ≥ 1 entry to the per-league adversarial corpus in `tests/adversarial/per_league/`. The corpus covers at least: a truncated response for the league's primary source, an encoding-error response, and a schema-drift response (field renamed). `test_19_9_per_league_adversarial_corpus_minimum.py`.
...
- [x] **Phase 18 conformance on every onboarding PR.** `xops/lint/phase18_conformance.py` runs in CI on every PR that touches `common/league_catalog.yaml`, `common/leagues/`, or `infra/mock/seeds/<source>/`. Failure blocks merge — verified by `test_19_12_conformance_runs_on_onboarding_pr.py`.
...
- [x] **Backward-compatible read path.** `common/league_catalog_loader.py` can read both v1 and v2 files; it auto-invokes the migrator in memory for v1 files (no file write). The migrated representation is cached in the msgpack cache (§19.7) as v2. `test_19_16_loader_reads_v1_with_migration.py`, `test_19_16_loader_caches_migrated_v2.py`.
...
- [x] **Freshness and trigger rules use `kickoff_utc` exclusively.** `xops/lint/no_local_time_in_freshness.py` asserts no freshness rule or retrain trigger in `swarm/source_watcher/` or `common/` compares a wall-clock value (e.g. `datetime.now()`, `time.time()`) against `kickoff_local.datetime` directly. All comparisons must go through `kickoff_utc`. `test_19_19_freshness_rules_use_utc.py`.
...
- [ ] `MONETIZATION_ENABLED` env var registered in both `common/config.py` and `server/internal/config`; default `false`.
...
- [ ] `common/betting_markets.json` v2 schema: every market has a `family` field.
...
- [x] Register four new keys in `xops/versioning/chart.json` (hand-edit only, then `make version.validate` green): `enrichment_roster`, `enrichment_health`, `enrichment_officials`, `enrichment_environment` — all seeded at `0.1.0` with `will_reach_1_0_0_in_phase: 21`.
- [x] Add all feature-flag tunables to `common/config.py` and `xops/env/.env.example` (documented with type, default, and description; `no_magic.py` lint must pass):
...
- [x] Add all numeric tunables to `common/config.py` and `xops/env/.env.example`:
...
- [x] Schema: `TransferPayload`, `ContractPayload`, `SuspensionPayload` per `ENRICHMENT_DATA.md §2.1`. `confidence: Literal["rumour", "agreed", "official"]` present on all three types.
- [x] Extractor: `datasource/scraper/extractors/transfers_feed/` — parses club-site announcement markup and Mackolik transfer fragment. Invalid rows raise `ExtractionError` (never silently return `None`); all string fields are length-capped before storage.
- [x] Differ: `datasource/refresher/diffs/transfers_feed/` — diff key is `(player_id, effective_at, confidence)` triple; emits a diff event only when at least one field changes; idempotent on re-fetch of identical data.
- [x] Confidence gate (enforced in the storage writer, not just the extractor):
...
- [x] Feature computation (computed in `model/features.py`; no hardcoded coefficients — all from config §21.0):
...
- [x] Transfer-window scheduler: daily cadence Jul 1–Sep 1 and Jan 1–Feb 1 (configurable per confederation in `cfg.enrichment_roster_cron`); weekly cadence outside windows. The scheduler emits a heartbeat event even when no transfers are found (so monitoring can distinguish "no news" from "pipeline stalled").
- [x] Suspension sub-records: competition-scoped; `matches_remaining` decremented by a post-match reactor; record expires when `expires_after_match_id` resolves AND `matches_remaining` reaches 0.
- [x] **Cross-source deduplication**: a transfer arriving from two sources (e.g., Mackolik feed and club site announcement) for the same `(player_id, transfer_type, effective_at)` triple is collapsed to a single record before any write. The higher-`confidence` source wins; if confidence is equal, the source defined earlier in `xops/mock/sources.py` has priority (deterministic, testable). This prevents phantom duplicate features in the squad-strength computation. The deduplication window is the entire differ cycle, not a time window.
- [x] Tests — `test_21_1_roster_state.py` (≥ 8 tests, including ≥ 2 adversarial):
...
- [x] Schema: `InjuryPayload`, `AvailabilityPayload` per `ENRICHMENT_DATA.md §3.1`. `source_url_hash` is SHA-256 of the canonical source URL (provenance preserved without storing the raw URL; hashing policy documented in `ENRICHMENT_DATA.md §3.1`).
- [x] Extractor: `datasource/scraper/extractors/injury_watch/` — parses Mackolik injury list and club presser fragments. Returns validated `InjuryPayload` or `AvailabilityPayload`; invalid records raise `ExtractionError`.
- [x] Differ: `datasource/refresher/diffs/injury_watch/` — diff key `(player_id, fixture_id, status, source_confidence)`; higher-confidence source overrides lower within the same fixture window (`club_official` > `manager_presser` > `press` > `rumour`).
- [x] Confidence-override rule: a `club_official` status in the past 24h overrides all other statuses for the same player + fixture, regardless of recency.
- [x] Stale-decay rule: `doubtful` status older than 36h before KO auto-decays to `fit` for prediction purposes; editorial flag stays `uncertain` in the response.
- [x] Post-match truth correction: reactor fires after lineup confirmed; any player in `starting_xi` or `substitutes` is retroactively set to `fit` for that fixture. Reactor is **idempotent** — re-running it on an already-corrected record is a no-op.
- [x] Per-fixture squad availability vector: `team_strength` feature reduced by Σ(unavailable_player_rating × starter_likelihood) for all non-`fit` statuses. Proofreader CI bounds are widened when > 30% of the modelled starting XI has `doubtful` status (threshold from `cfg.enrichment_health_ci_widen_threshold`, default 0.30).
- [x] International window calendar: confederation break windows tracked; `international_duty` status does not count as an injury for squad-strength penalty (avoids penalising teams whose players are called up, not hurt).
- [x] Tests — `test_21_2_health_plane.py` (≥ 7 tests, including ≥ 2 adversarial):
...
- [x] Schema: `RefereeAssignmentPayload`, `RefereeProfilePayload` per `ENRICHMENT_DATA.md §4.1`. `rolling_stats` sub-dict is populated exclusively by the reactor, never by the extractor.
- [x] Extractor: `datasource/scraper/extractors/referee_reg/` — parses TFF referee assignment page and UEFA/FIFA assignment feeds. Validates that `main_referee_id` resolves to a known `RefereeProfilePayload` before writing; raises `ExtractionError` if the referee is unknown.
- [x] Differ: `datasource/refresher/diffs/referee_reg/` — diff key `(fixture_id, main_referee_id)`; `last_minute_change` is set to `true` when a reassignment arrives < 24h before KO.
- [x] Rolling-stats reactor: recomputes all `rolling_stats` fields for `referee_id` on every officiated-fixture `score` event; **debounced** — multiple score events for the same referee in one batch produce exactly one recompute (per `CONTENT_FRESHNESS.md §15`).
- [x] Home-bias correction: when `home_win_pct > cfg.referee_home_bias_clamp`, a per-fixture Elo nudge is applied at predict-time; the nudge is clamped and never compounds across re-predictions for the same fixture.
- [x] `last_minute_change=true` flow: invalidates cached cards/penalty derived features for that fixture and enqueues a recompute with the new assignment within one scrape cycle.
- [x] Assignment uniqueness: `(fixture_id)` is a unique constraint in Postgres; a duplicate assignment for the same fixture is treated as an update (last-write-wins within a 5-min **writer-side** deduplication window keyed by `(fixture_id, main_referee_id, announced_at)` — this is distinct from the 24-hour `last_minute_change` detection window). The two windows serve different purposes: the 5-min writer window prevents burst duplicates within a single scrape cycle; the 24h window detects genuine last-minute reassignments between separate scrape cycles. A reassignment from referee A to referee B that arrives within 5 min of the previous write is still a `last_minute_change=true` event if the gap between the original `announced_at` and the new `announced_at` is < 24h.
- [x] **Rolling-stats warm-start**: when the Officials plane is first deployed in a fresh environment (zero `referee_profiles` exist), historical match records already present in the Live plane (plane 3) are replayed to bootstrap `rolling_stats` for every referee found therein. The warm-start replay respects the `cfg.enrichment_referee_window_matches` (default 50) window and produces correct per-referee stats even before the first live scrape cycle. The warm-start is invoked by `make enrichment.bootstrap` (§21.22) and is also triggered automatically on the first call to the Officials extractor when the `referee_profiles` table is empty. **Without the warm-start, all referees start with zero rolling stats regardless of match history** — this would make the Officials plane useless on first deployment and corrupt the cards/penalty feature columns.
- [x] Tests — `test_21_3_officials_plane.py` (≥ 8 tests, including ≥ 2 adversarial):
...
- [ ] Per-plane flags (config stubs from §21.0) are enforced at predict-time in `model/features.py`. When a plane is disabled:
...
- [x] `make calibration.enrichment` target added under `xops/makefile/calibration.py` — runs the full calibration harness outside the normal test suite; writes `data/enrichment_baseline.json` and `data/enrichment_fallback_values.json`; exits non-zero if any plane fails the logloss delta threshold.
- [x] Held-out evaluation harness `tests/test_21_enrichment_calibration.py` measures log-loss delta from enabling each enrichment plane incrementally on the same held-out match window used in Phase 5b.
- [x] Log-loss baseline (planes 1–5 only) is computed once and written to `data/enrichment_baseline.json`; CI asserts the baseline is reproducible within ± 0.001 log-loss across re-runs (no random seed variance). **Seeding**: the baseline computation uses random seed `cfg.enrichment_calibration_seed` (default 42; configurable via `ENRICHMENT_CALIBRATION_SEED` in `common/config.py`); the seed is fixed per-run to ensure reproducibility across machines. The held-out window is the same time slice used in Phase 5b (`cfg.calibration_holdout_start` to `cfg.calibration_holdout_end`); those keys are already in config and shared by both sub-phases.
- [x] Per-plane delta must individually meet `cfg.enrichment_promotion_logloss_delta ≥ 0.005` before that plane is declared production-ready. A plane that does not meet the threshold is flagged `degraded` in `data/enrichment_baseline.json`; CI marks that plane's §21.0 chart key as `0.x.0` (not promoted to `1.0.0`).
- [x] The combined delta (all four planes + all four derived views enabled) is measured and compared to the individual deltas; regression from combining is flagged.
...
- [x] **Feature distribution stability check**: after the calibration run, `make calibration.enrichment` asserts that no enrichment column is >95% zero-valued in the held-out dataset. A column that is >95% zero has been computed incorrectly (signal never arrived, the extraction or feature computation is broken). Failing columns are listed in `data/enrichment_baseline.json` under `"zero_dominated_columns"` and block plane promotion even if logloss meets the threshold.
...
- [x] Cards / corners / fouls markets (Phase 13a) gated to **`pro`** tier; require Officials plane (8) + fixture-congestion derived view to be active.
- [x] Player-prop markets gated to **`premium`** tier; require Roster plane (6) + Health plane (7).
- [x] Weather-sensitive special markets (`windy_day_totals`, `red_card_markets`) gated to **`premium`** tier; require Environment plane (9).
- [x] Card-context overlay computed only for `premium` subscribers; enforced in the Go API's enrichment middleware (`server/internal/enrichment/`), not in the AI pipeline — the AI layer always computes it when the planes are up; the API layer gates the response.
...
- [ ] New entity classes added to the NLP lexicon (`nlp/lexicon/` YAML files):
...
- [ ] Turkish sample queries for all six intents added to `tests/fixtures/turkish_queries.yaml` (≥ 5 samples per intent, including negation variants and plausible misspelling variants).
- [ ] Response templates for all six intents in `trc/` — Turkish-language output, English-infra naming convention.
...
- [ ] **Turkish query minimum floor**: `tests/fixtures/turkish_queries.yaml` must have ≥ 5 samples (including ≥ 1 negation and ≥ 1 plausible misspelling) for **each** of the six new intents. A CI test reads the YAML and fails if any intent falls below the floor — the YAML is authoritative, not the test count.
...
- [ ] Feature column additions to `model/features.py` and `common/constants.py`:
  ...
  - **Total enrichment columns: 27**. `N_FEATURES` in `common/constants.py` updated from 120 → **147**. *(Earlier drafts stated 25 / 145; those counts omitted `drift_total_pct` and `implied_prob_shift_max` from the market-movement view. The correct canonical count is 27 enrichment columns, yielding 147 total.)*
- [ ] `FEATURE_COLUMNS` list in `model/features.py` updated; existing 120 columns unchanged (enrichment columns appended as a block, never interleaved — preserves backward-compat shim path for consumers that load a saved model trained with 120 features). `N_FEATURES` is derived as `len(FEATURE_COLUMNS)` and therefore auto-resolves to 147; the constant must **not** be hardcoded to a literal integer — the existing `N_FEATURES: int = len(FEATURE_COLUMNS)` pattern is the correct form (a test asserts `N_FEATURES == 147` after §21.16 ships, as a snapshot regression guard).
- [ ] `EnrichmentSource` Protocol added to `model/features.py`: a dependency-injectable interface with `get_enrichment(fixture_id: str, plane_id: str, league_id: str) -> dict[str, float]` and `get_enrichment_batch(fixture_ids: list[str], plane_ids: list[str], league_id: str) -> dict[str, dict[str, float]]`. In-memory dict backend for tests; Redis-backed backend for prod. The Protocol is constructor-injected into `extract_features_for_match()` — no global state.
...
- [ ] `make lint` green — `no_magic.py` passes (no hardcoded numeric thresholds, no raw provider URLs, all config keys via `common/config.py`).
...
- After Phase 22 (restructure), the v0.2 synthetic formulas and their test fixtures are candidates for removal; they are marked `# DEPRECATED: superseded by enrichment plane <N>` in `model/features.py` in this sub-phase.
- `ENRICHMENT_DATA.md` §9 feature-flag table updated to note which v0.2 columns each plane supersedes.

- [ ] All eight v0.2 proxy columns annotated `# DEPRECATED: superseded by enrichment plane <N>` in `model/features.py`.
- [ ] `generate_synthetic_dataset()` in `model/features.py` still produces valid synthetic values for the superseded columns (used when all planes are disabled during test isolation).
...
- [ ] **Backfill mode flag**: `ENRICHMENT_BACKFILL_MODE=false` added to `common/config.py` and `xops/env/.env.example`. When `true`, the enrichment pipeline runs in backfill mode; circuit breakers are suspended; DLQ writes are suppressed. This flag is never enabled in production except by an explicit operator command.
...
- [ ] Add config keys to `common/config.py` and `xops/env/.env.example` (**already declared in §21.0**; no additional declaration needed here — this bullet serves as a cross-reference only):
...
- [ ] **Consistency invariant definitions** (`datasource/enrichment/consistency.py`):
...
- [ ] Add config keys to `common/config.py` and `xops/env/.env.example`:
...
- [ ] **Per-plane Prometheus counters and gauges** registered in `common/telemetry.py` (using the existing `TelemetrySink` API):
...
- [ ] A **smoke corpus fixture** (`tests/fixtures/smoke_enrichment_fixture.json`) is committed: a single synthetic match with enough pre-populated enrichment data (one transfer, one injury, one referee assignment, one weather forecast) to exercise all enrichment-derived feature columns in a single smoke run.
...
- [x] `migrations/018_enrichment_audit.sql` applied; append-only constraint enforced; §21.25 tests green (10/10 passing).
...
- [x] **Feature store**: `FEATURE_COLUMNS` extended to **157 entries** (120 original + 10 QID + 27 enrichment); `N_FEATURES` auto-resolves to **157** via `len(FEATURE_COLUMNS)`; **27 enrichment columns** appended (never interleaved; market-movement contributes 6 columns, not 4 — see §21.16 for the corrected breakdown); Redis key format is `f"enrich:{league_id}:{fixture_id}:{plane_id}"`; batch `mget` API via `EnrichmentSource.get_enrichment_batch()`; `msgpack` serialization (~30-40% size reduction); NaN-freedom proof green; §21.16 + §21.24 tests green (7/7 passing).
- [ ] **Schema registry**: all 10 enrichment `record_type` enum values present; 10 JSONSchema files in `common/schemas/feeds/`; `make schema.validate` green; §21.17 tests green.
...
- [x] **Model compatibility**: `ModelShim` for 120-feature saved models tested; retraining gate detects all-four-planes-at-`1.0.0` and emits the retrain event; `make train.enriched` produces `models/predictor_v157.pkl`; §21.23 tests green (9/9 passing); backward-compat feature slicing for old models verified.
- [x] **Performance**: enrichment batch fetch P99 ≤ 10ms; total enrichment overhead P99 ≤ 30ms; `make bench.enrichment` exits zero; §21.24 perf tests green (7/7 passing); msgpack serialization efficiency proven (30-40% size reduction).
- [ ] `make lint` green — `no_magic.py` passes; no hardcoded numeric thresholds, no raw provider URLs, no hardcoded league_id strings; all config keys via `common/config.py`.
...
root-level `datasource/` stub from the Python package tree. Every Python
package that today lives under `ai/` moves to the repo root; all imports,
tests, Dockerfiles, Makefiles, CI workflows, k8s manifests, `pyproject.toml`,
per-component lockfiles/SBOMs, Grafana dashboards, alert rules, Go path
constants, and documentation references are updated by automated tooling.
This is the final structural step; the codebase must be
**100% correct, fully tested, and 30-day burn-in clean** before this phase
closes.

> **📐 Verified inventory baseline (audit 2026-06-20 — refresh with
> `make phase22.inventory` at execution time; these numbers drive the
> ledger and the move plan).** The codemod must be re-pointed at the live
> counts before any commit — the figures below are the *audited starting
> point*, not a contract:
>
> | Fact | Verified value | Source of truth |
> |---|---|---|
> | Python files under `ai/` | **1 059** | `find ai -name '*.py' \| wc -l` |
> | `ai/` files importing `ai.*` | **458** | `grep -rlE 'from ai\.\|import ai\.'` |
> | non-`ai/` Python files importing `ai.*` | **55** | same grep, `grep -v '^ai/'` |
> | non-`ai/` importer breakdown | **44 `xops/`, 10 `common/`, 1 `docs/`** | per-dir `uniq -c` |
> | overlapping `common/` sub-packages | **5** (`bus`, `db`, `isolation`, `observability`, `security`) | `ls common/ ∩ common/` |
> | root `common/` sub-packages that are **shims re-exporting `.*`** | **`bus`, `config`** (authoritative copy is in root) | `grep 'from [a-z_]' common/<sub>/` |
> | `Makefile` `PYTHONPATH=.` occurrences | **2** (not "7+") | `grep -n 'PYTHONPATH=\.' Makefile` |
> | `conftest.py` files in the tree | **6** (root, `common/tests`, `tests`, `swarm/agents/tests`, `swarm/agents/maint/tests`, `xops/backup/tests`) | `find . -name conftest.py` |
>
> **There are no `swarm/` or `server/` Python importers of `ai.*`** — `server/`
> is Go and references `ai/` only in comments, one runtime path constant, and a
> parity-test source-path table (§22.5). Any per-package "~N files" figure in
> the draft move plan is superseded by the live `make phase22.import-report`
> output; the ROADMAP prose is informational, the report is authoritative.
...
> **🕓 Phase 17 ships *after* Phase 22 (binding sequencing fact).** The
> Scraper-Patcher (Phase 17) is explicitly unblocked *by* this phase (see
> §22.DoD). At Phase 22 execution time the patcher/gitops packages do **not**
> exist (`datasource/patcher`, `datasource/gitops`, `xops/patcher/cassettes`,
> `feeds/ops/bundles` are all absent — verified 2026-06-20). Therefore every
> patcher-artifact migration step in this phase (§22.12 cassettes/bundles,
> ledger #24) is **conditional**: it migrates artifacts *if present* and
> asserts *empty if absent*. The `datasource_patcher` / `datasource_gitops`
> `1.0.0` bumps in §22.10 are likewise deferred to Phase 17 unless those
> packages have already landed. Do not write a migration that assumes Phase 17
> artifacts exist.
...
>
> 🧰 **Automated tooling is mandatory.** 1 059 Python files under `ai/` — 458
> of which contain `from ai.` imports — plus 55 non-`ai/` files (44 in `xops/`,
> 10 in `common/`, 1 in `docs/`) that also import from `ai.*` cannot be safely
> rewritten by hand. `libcst`-based codemod scripts
> (`xops/codemod/phase22_rewriter.py`, dispatched via `xops/makefile/phase22.py`)
> perform all import rewrites; diffs are reviewed, not authored. Non-Python
> artefacts the codemod must **not** touch (per-component `requirements.lock`,
> `sbom.spdx.json`, compiled `__pycache__`) are handled by dedicated
> migration steps (§22.5, §22.11), not the import rewriter.
...
8. **Non-`ai/` callers migrate in the same commit** as their target package. The
   55 non-`ai/` files that import from `ai.*` (44 `xops/`, 10 `common/`, 1 `docs/`)
   must be co-updated by the codemod in the same PR as the package they reference.
   Leaving them to a later "catch-up" PR creates an import-error window where
   those packages fail to import for callers.
9. **Isolation snapshot is refreshed per commit**, not per phase. §22.4 makes
   this a per-step gate; skipping any refresh causes false-positive isolation
   violations in subsequent CI runs.
10. **Per-component lockfiles & SBOMs travel with their package.** `swarm/`
    and `datasource/` each carry a `requirements.lock` + `sbom.spdx.json`
    (Phase 18 §18.10 artefacts) whose internal paths and provenance reference
    `ai/`. The codemod never edits them; instead each move regenerates them via
    `make lockfiles.regen` + `make sbom.gen` for the new root path and passes the
    Phase 18 §18.10 lockfile-review gate **in the same commit**. A move that ships
    a stale lockfile/SBOM is reverted.
11. **No stale bytecode crosses the move.** `git mv` does not move `__pycache__/`,
    but a developer's working tree may carry stale `ai/**/__pycache__/*.pyc` that
    shadows a fresh import after the move. Every move commit runs
    `find . -name '__pycache__' -prune -exec rm -rf {} +` before the test gate;
    CI runs with `PYTHONDONTWRITEBYTECODE=1`.
12. **No-clobber, no-loss moves (binding).** No `git mv` uses `-f`/`--force`. A
    pre-move assertion confirms the destination path does **not** already exist
    for a verbatim move, **or** is an explicitly-decided merge target with a
    per-file resolution recorded in `docs/decisions/phase22/`. Every move/merge
    commit is gated by three mechanical "nothing-lost" checks (ledger #43–#45):
    (a) **file-accountability** — every `ai/` source file lands at exactly one
    destination or is deleted-with-reason, and no destination receives two
    sources without a recorded decision; (b) **symbol-union preservation** — the
    post-merge module's public surface is a superset of the union of both source
    copies' public surfaces, minus an explicit `dropped_symbols.md` allow-list
    (each entry carrying a no-live-caller proof); (c) **content-hash
    preservation** — every verbatim-moved file is byte-identical (blob SHA
    unchanged) except the intended codemod import rewrites. `make test` green is
    *necessary but not sufficient* — it proves tested code still imports, not that
    every public symbol survived.
...
The Phase 22 design baseline grew by retiring specific *wrong* assumptions
surfaced by prior migration attempts, ops dry-runs, code analysis, and
architecture reviews. Each row is **binding**: a future PR that silently
restores the wrong assumption must add a new row explaining why the old
reasoning no longer applies and must ship a green proof test in the same diff.
`xops/lint/phase22_ledger.py` enforces density (no row gaps) and append-only
invariant across this table.
...
| 2 | "`common/` and `common/` have no overlapping Python modules." | `common/logger.py` and `common/international_tournament_profiles.py` already exist at the repo root as top-level files (different, simpler versions). The **overlapping sub-packages are exactly five** — `bus/`, `db/`, `isolation/`, `observability/`, `security/` exist in *both* `common/` and `common/` (verified 2026-06-20). `common/feeds/` and `common/api/` are **`ai`-only** (no root counterpart) — they are *move-ins*, not merges, and the draft's claim that they overlap is wrong. A naive `git mv` over any of the five real overlaps silently overwrites the Phase-18-governed root copies, destroying the isolation gates. Each of the five requires an explicit diff-audit and an authoritative-copy decision (which copy wins is **heterogeneous** — see ledger #36). | `test_22_0_common_overlap_is_exactly_five_subpackages.py`, `test_22_0_feeds_and_api_are_ai_only_move_ins.py` |
| 3 | "`ai/tests/` → `tests/` is a simple folder rename." | Root `conftest.py` already exists and detects the Phase 22 layout via `_phase22_layout = (ROOT / "common" / "config" / "__init__.py").exists()`. `ai/tests/conftest.py` performs its own `sys.path.insert(0, _ai_path)` and also clears `sys.modules` of stale `common.*` entries. After the move both conftests are in scope for the same test session; conflicting `sys.path` mutations and `sys.modules` clears cause non-deterministic import resolution and test isolation failures. All conftest files must be audited and explicitly consolidated — not mechanically concatenated. | `test_22_0_conftest_hierarchy_conflict_detected.py`, `test_22_0_merged_conftest_no_sys_modules_clear.py` |
| 4 | "The root `datasource/` folder only contains `quarantine.py` and can be deleted as-is." | `datasource/quarantine.py` is the only file at root, but `datasource/` is a completely different sub-tree with `enrichment/`, `t3_resource_manager.py`, patcher context, and `__init__.py`. The two `datasource` names coexist under different roots. After migration `enrichment/` and `t3_resource_manager.py` become root-level packages; `datasource/quarantine.py` moves to `scraper/quarantine.py`. Treating the root `datasource/` as "empty or ignorable" orphans the entire enrichment sub-tree. | `test_22_0_datasource_folders_both_inventoried.py`, `test_22_0_ai_datasource_enrichment_lands_at_root.py` |
| 5 | "`PYTHONPATH=.` is already the universal setting across all tooling." | The `Makefile` contains **2** explicit `PYTHONPATH=ai` overrides (verified: lines for `test.phase12` and `test.adversarial_corpus`; *not* the "7+" an earlier draft guessed). `pyproject.toml` lists `"ai"` in `known_first_party` **and** in `coverage.run source`. `common/config/__init__.py` and `common/bus/` re-export from `ai.*` transitionally. Leaving any `PYTHONPATH=ai` occurrence (Makefile, CI, compose, k8s) active after the move causes those targets to import from the post-deletion void. Every occurrence must be enumerated from the live tree (not guessed), migrated, and lint-gated. | `test_22_0_pythonpath_ai_occurrences_enumerated.py`, `test_22_0_no_pythonpath_ai_after_migration.py` |
| 6 | "Docker layer cache self-heals after file paths change." | BuildKit caches layers by content hash. A `COPY ai/ ./ai/` directive that becomes `COPY . .` changes the COPY instruction itself, busting all cached layers. The first post-migration build is a full cold build. Additionally, `.dockerignore` must explicitly exclude `xops/`, `docs/`, `data/`, `infra/`, `migrations/`, and `server/` to prevent bloating the Python image with non-Python artefacts. Post-migration image size must be verified within ±10% of the pre-migration baseline. | `test_22_0_docker_image_size_within_tolerance.py`, `test_22_0_dockerignore_excludes_non_python_dirs.py` |
| 7 | "`common/schemas/` has no root `common/` counterpart — it moves wholesale without governance." | `common/schemas/` is not yet listed as an explicit Phase 18 governed sub-package in `common/SUBPACKAGE_CHARTER.md`. `common/schemas/` contains JSONSchema feed definitions, the `records.py` enum (extended by Phase 21 to 10 enrichment types), and generated TypedDicts. Moving it requires triple CODEOWNERS ACK and a new sub-package charter entry per Phase 18 §18.13. The move must be gated on that ACK before any file is touched. | `test_22_0_common_schemas_subpackage_charter_exists.py`, `test_22_0_schemas_move_requires_codeowner_ack.py` |
| 8 | "The isolation snapshot can be refreshed after the move as a cleanup step." | `common/isolation/import_graph.snapshot.json` was built against the `ai/` transitional layout. Any CI run between a package move and the snapshot refresh will compare the updated import graph (root-level packages) against the stale snapshot (`ai.*` references) and generate false-positive isolation violations that block unrelated PRs. The snapshot MUST be refreshed atomically in the same commit as each package move. | `test_22_0_snapshot_refreshed_after_each_move_commit.py`, `test_22_0_snapshot_stale_blocks_ci.py` |
| 9 | "`test_ai_tree_gone.py` currently passes — it may be accidentally ticked." | The test asserts `not (ROOT / "ai").exists()` and **fails** in CI as long as `ai/` exists. Ticking the Phase 22 DoD checkbox before §22.7 runs is a false completion signal. The test is a forward-looking regression gate that deliberately fails until the deletion commit lands. Never tick the DoD item until the test is green. | `test_22_0_tree_gone_gate_correctly_fails_before_deletion.py` |
| 10 | "Circular imports cannot arise from merging `common/` and `nlp/` to root level." | `common/config.py` has a conditional import of `ai.common.season`; `common/telemetry.py` imports `ai.common.config`. `nlp/` imports `common.constants`. After the move Python resolves `common.constants` before `nlp/` initialises; but `common.constants` may import from `nlp/` for NLP-specific constants. Even without a cycle, import ordering changes when the `ai.` prefix is removed, because Python now resolves both under the same root namespace. The codemod must run `pydeps` import-order analysis before and after each merge; any detected cycle is a hard stop. | `test_22_0_no_new_circular_imports_after_each_merge.py`, `test_22_0_pydeps_baseline_committed.py` |
| 11 | "Rollback is `git revert <move-commit>`." | Structural refactor commits are large and dense; `git revert` frequently produces merge conflicts in files that were also touched by concurrent feature PRs. The correct rollback path is: (a) reverse-order per-step `git revert` of the migration commits, (b) `make isolation.check --full`, (c) `PYTHONPATH=. make test`, (d) snapshot refresh. `make phase22.rollback STEP=N` automates all four steps. Two consecutive failed rollback attempts within one step trigger a migration pause and escalation. | `test_22_0_rollback_target_registered_for_each_step.py`, `test_22_0_rollback_reruns_isolation_check.py` |
| 12 | "The `source_watcher → datasource_watcher` alias cleanup is automatic." | The alias window was activated at a specific calendar date. If Phase 22 runs more than 90 days after activation, the window has expired; the old key should be `eol` already. If it runs within the window, the CLI translation layer must be verified active before EOL is flipped. Neither transition is automatic — Phase 22 §22.10 explicitly executes the EOL flip and verifies the translation layer preserves historical tracker queries. | `test_22_0_source_watcher_alias_eol_executed_in_phase22.py`, `test_22_0_historical_tracker_queries_resolve_via_translation.py` |
| 13 | "k8s manifests in `infra/k8s/` do not embed Python paths." | k8s ConfigMap objects, Deployment spec `command:` / `env:` blocks, and init-container specs may contain `PYTHONPATH=ai`, `python ai/main.py`, or `mountPath: /app/ai` volume references. These are YAML documents not caught by Python AST or grep on `*.py` files. A dedicated YAML-aware scanner (`xops/lint/phase22_k8s_paths.py`) must run over all files in `infra/k8s/`, `infra/redis/`, and any Helm chart values files, and must be wired into CI before any file moves. | `test_22_0_k8s_manifests_scanned_for_ai_paths.py`, `test_22_0_k8s_scanner_wired_in_ci_before_first_move.py` |
| 14 | "Generated file headers (`negelir-generated-from: <path>@<sha256>`) are stable across the move." | A small set of files carry a `# negelir-generated-from:` provenance header (audited: **3** such files under `ai/` as of 2026-06-20). The previously-assumed locations `common/schemas/_generated/` and `swarm/clients/server_internal/` **do not exist** — do not migrate phantom directories. For the real generated files, the descriptor path inside the header becomes stale after the move (`common/...` → `common/...`); the Phase 18 §18.11 generation-parity lint fails on the stale path. The codemod rewrites provenance headers (pattern 7); `make codegen` is re-run to regenerate against the new descriptor path; the parity lint runs before the commit. The live generated-file set is discovered by `make phase22.inventory`, never hard-coded. | `test_22_0_generated_file_headers_updated_after_move.py`, `test_22_0_codegen_rerun_after_descriptor_move.py` |
| 15 | "`requirements.txt` merge is safe as a file concatenation." | `ai/requirements.txt` exists; no root-level `requirements.txt` exists today, but `ai/requirements-dev.txt` adds dev-only pins. Simply moving `ai/requirements.txt` to the root and adding the dev file risks introducing version specifier drift from any future root-level additions. The merge must use `pip-compile` on a combined `.in` file; the output `requirements.lock` must pass the Phase 18 §18.10 SBOM diff gate before any container build references the new lockfile. | `test_22_0_requirements_merge_uses_pip_compile.py`, `test_22_0_requirements_lock_passes_sbom_gate.py` |
| 16 | "Phase 17 ROADMAP bullets referencing `patcher/` are just text — no runtime effect." | `xops/lint/phase17_ledger.py` validates that filesystem paths cited in ROADMAP Phase 17 bullets resolve to real paths. After the move `patcher/` becomes `patcher/`; the lint rejects the stale paths and blocks unrelated PRs until §22.12 updates all references. This must be coordinated — §22.12 runs the path sweep and validates the ledger lint before declaring done. | `test_22_0_phase17_ledger_lint_passes_after_path_update.py`, `test_22_0_phase18_ledger_lint_passes_after_path_update.py` |
| 17 | "Dead-code analysis is optional — whatever was in `ai/` comes to root as-is." | Bringing unreachable symbols to root inflates every root package's public surface, making Phase 18's `__all__`-based isolation gate harder to enforce and triggering false positives in `vulture`'s post-migration pass. Any dead symbol that also appears in a patcher bundle scope contract causes a false-positive patcher artifact rejection after the move. A `vulture` scan of `ai/` must be committed to `docs/tracking/phase22_dead_code_candidates.md` **before the first package move**; confirmed dead code must be removed in dedicated cleanup PRs. | `test_22_0_vulture_dead_code_scan_committed.py`, `test_22_0_no_confirmed_dead_symbols_at_root.py` |
| 18 | "The 1.0.0 version bumps are a formality — just run `make version.bump`." | Phase 18 §18.5 requires each component reaching `≥ 1.0.0` to have (a) `<component>/PUBLIC_API.md` (CODEOWNERS-protected, auto-generated from `__all__`), (b) `<component>/tests/test_public_api_compat.py` pinning the surface, and (c) the deprecation policy documented in `docs/coding/component_versioning.md`. `xops/versioning/version.py` refuses the bump without all three; running `make version.bump` without shipping the artefacts first always fails. §22.10 ships the artefacts for all six components before bumping. | `test_22_0_version_bump_refused_without_public_api_doc.py`, `test_22_0_all_six_components_have_compat_test_before_bump.py` |
| 19 | "The stop-the-world migration is feasible — it's a mechanical search-and-replace." | At time of writing: ~1 059 Python files under `ai/` × average ~4.2 `from ai.` import lines per file that needs rewriting = ~4 500 individual rewrites. Plus ~55 non-`ai/` files. A single PR of this size is un-reviewable, un-rollbackable in practice, and blocks all parallel feature work. The per-package incremental approach (§22.4) keeps each PR under 150 changed files; each is independently rollbackable in under 5 minutes. | `test_22_0_migration_uses_incremental_per_package_commits.py`, `test_22_0_each_migration_pr_under_150_files.py` |
| 20 | "AI-layer metric names carry no package path identifiers — renaming is not needed." | `common/telemetry.py` registers metrics whose names were constructed from the module path at definition time, producing `ai_pipeline_*`, `ai_scraper_*`, and `ai_model_*` prefixes. After the move the same code lives in `pipeline/`, `scraper/`, and `model/` — any consumer of those metrics that constructs the name dynamically from `__name__` or `__module__` will emit a different label, silently breaking Grafana dashboards and alert rules. §22.9 freezes all metric names into string literals and renames non-conforming ones before the packages move. | `test_22_0_no_ai_prefixed_metric_names_after_migration.py`, `test_22_0_metric_names_are_string_literals_not_dynamic.py` |
| 21 | "The conftest.py files can be merged by concatenating them without inspection." | Root `conftest.py` contains Phase 22-aware layout detection (`_phase22_layout`) and the final target marker registrations. `ai/tests/conftest.py` manipulates `sys.path` and clears `sys.modules` entries — behaviours that must be removed after the move. Both register `pytest.mark` markers; duplicates produce warnings that degrade test output readability. The merge requires explicit four-step resolution: (1) retain layout detection logic, (2) strip stale `sys.path` manipulations, (3) remove `sys.modules` cache clears, (4) deduplicate marker registrations. | `test_22_0_merged_conftest_has_no_duplicate_marker_registrations.py`, `test_22_0_merged_conftest_has_no_sys_path_ai.py` |
| 22 | "After the move `import ai` still works as a backward-compat shim for one release cycle." | `ai/` must NOT be importable as a Python namespace after Phase 22 completes. Any surviving `import ai` signals: a Phase 17 patcher bundle recording the wrong module path, a stale cassette, or a test fixture that was not updated. The deletion in §22.7 enforces this; `test_ai_tree_gone.py` proves it; the resurrection lint prevents re-creation. The shim window (DeprecationWarning from shim re-exports) applies during §22.4–§22.7, not after §22.7. | `test_22_0_import_ai_fails_after_deletion.py`, `test_22_0_no_shim_window_after_deletion.py` |
| 23 | "Type annotation strings like `'ai.common.Config'` are handled by the import rewriter." | Forward-reference strings in `TYPE_CHECKING` blocks, `Annotated[...]` metadata, and Pydantic `model_rebuild()` calls contain quoted type names that are `SimpleString` / `ConcatenatedString` nodes, not `Import` / `ImportFrom` nodes. The `libcst`-based codemod must separately walk all string node types and rewrite `'ai.*'` annotation strings. A test corpus of ≥ 15 annotation patterns verifies the codemod handles all of them without corrupting non-annotation string literals (e.g. SQL query strings that contain `ai.` by coincidence). | `test_22_0_codemod_rewrites_annotation_string_literals.py`, `test_22_0_codemod_does_not_rewrite_sql_strings.py` |
| 24 | "Patcher cassettes and bundle metadata are unaffected by the move." | Phase 17 patcher cassettes under `xops/patcher/cassettes/` record file paths in tool-call outputs (e.g. `Read: scraper/extractors/mackolik/...`). After the move these paths are stale; the cassette drift probe (Phase 17 §17.6a) false-positives on every replay, exhausting the cassette drift budget. §22.12 runs `make patcher.cassette.migrate` which rewrites all cassette path strings, re-signs each cassette HMAC-SHA256 sidecar, and updates `MANIFEST.sha256`. Bundle metadata JSONs in `feeds/ops/bundles/` with `failing_code_path` referencing `ai/` are similarly migrated. | `test_22_0_patcher_cassette_paths_migrated.py`, `test_22_0_patcher_bundle_metadata_paths_migrated.py` |
| 25 | "The Phase 19 catalog loader shim contract is already satisfied." | Phase 19 §19.12 ships `common/league_catalog_loader.py` reading its path from `cfg.catalog_yaml_path`. Phase 22 §22.3a moves the loader to `common/catalog/league_catalog_loader.py`. The Phase 19 shim test (`test_19_12_catalog_loader_importable_via_shim_path.py`) must continue to pass throughout the shim window and post-deletion — the shim re-exports from the new path, not from `ai.*`. Failing to verify the shim contract produces a silent regression in the catalog API during the transition period. | `test_22_0_catalog_loader_importable_at_both_old_and_new_path_during_shim_window.py`, `test_22_0_catalog_loader_shim_emits_deprecation_warning.py` |
| 26 | "The root `common/config/__init__.py` already provides the canonical config — no migration needed." | `common/config/__init__.py` currently imports from `ai.common.config` transitionally (confirmed by `grep`). It is not a standalone config; it is a bridge that will break the moment `ai/` is deleted. The real config implementation lives in `common/config.py` and must be moved to `common/config/ai_pipeline.py` (§22.3a) before `common/config/__init__.py` is updated to re-export from the new path. Treating the root config as "already there" skips the move of ~1 000 lines of config logic. | `test_22_0_root_config_init_does_not_import_from_ai_after_migration.py`, `test_22_0_config_logic_at_canonical_path.py` |
| 27 | "The `common/isolation/` sub-package is the same as `common/isolation/`, and `dpa_validator.py` is the only net-new file the `ai/` copy contributes." | **Verified 2026-06-20 — both halves of this assumption are wrong.** `common/isolation/` contributes **zero net-new source files** (its only `ai`-unique entries are `__pycache__/*.pyc` bytecode); `dpa_validator.py` exists in **both** trees and **differs**. Every one of the four colliding files — `check.py`, `policy.py`, `dpa_validator.py`, `__init__.py` — **differs** between the two copies, so a blind "root wins" silently discards whatever the `ai/` versions added (Phase-19 DPA-aware logic), and a blind "ai wins" destroys the **root-only** files `go_check.go`, `graph_builder.py`, `forbidden_deps.yaml`, and `import_graph.snapshot.json` (the Phase 18 gate itself). The correct action is a **per-file diff-audit + symbol-union merge** for the four colliding files (not a discard), with the four root-only files preserved untouched. | `test_22_0_isolation_root_only_files_preserved.py`, `test_22_0_isolation_colliding_files_symbol_union_merged.py` |
| 28 | "Non-`ai/` files that import from `ai.*` will be caught by the codemod when their packages move." | There are **55** non-`ai/` files that import from `ai.*`, distributed as **44 in `xops/`, 10 in `common/`, 1 in `docs/`** (verified 2026-06-20). There are **no `swarm/` or `server/` Python importers** (`server/` is Go). These files' host packages — `xops`, `common`, `docs` — already live at root and are **not** scheduled to move in Phase 22's package-move phase. They remain broken with stale `ai.*` imports after §22.4 unless co-updated by the codemod in the **same commit** as the `ai/` package they reference (via `--include-non-ai-callers`). The §22.1 inventory enumerates them per-package alongside the `ai/` files. | `test_22_0_non_ai_callers_co_updated_with_target_package.py`, `test_22_0_inventory_includes_non_ai_callers.py` |
| 29 | "`CURRENT_SEASON` is a runtime config value — its hardcoding is harmless post-migration." | `common/constants.py` has `CURRENT_SEASON = "2025-2026"` as a module-level constant. Phase 22 §22.6 requires all such hardcoded season strings to be replaced by `cfg.current_season` (from `common/config/defaults.yaml`). If `CURRENT_SEASON` survives the migration as a root-level constant (now `constants.CURRENT_SEASON`), all downstream code that reads it will silently use the stale season after a season boundary, causing wrong freshness windows, retrain triggers, and schedule-plane queries. `xops/lint/no_hardcoded_season.py` must gate this before the constant moves. | `test_22_0_current_season_not_a_constant_after_migration.py`, `test_22_0_current_season_reads_from_config.py` |
| 30 | "The `N_FEATURES` constant is a fixed integer Phase 22 can hardcode in a snapshot test." | `N_FEATURES` is **derived** — `common/constants.py` carries `N_FEATURES: int = len(FEATURE_COLUMNS)`. There is a **live discrepancy to reconcile, not a number to assert**: the `constants.py` comment currently says “147 after Phase 21.16” while the Phase 21 canonical DoD (§21.26) says **157** (120 + 10 QID + 27 enrichment). Phase 22 must NOT hardcode either integer in any snapshot / model-compat test, must NOT edit the value during the move, and reconciles only the stale *comment* to match the actual `len(FEATURE_COLUMNS)` Phase 21 left in place. A divergence between `len(FEATURE_COLUMNS)` and the Phase 21 canonical value is a **Phase 21 defect** — recorded as a tracker row and fixed in Phase 21 scope, never silently patched during the move. | `test_22_0_n_features_is_len_feature_columns_not_hardcoded.py`, `test_22_0_n_features_matches_phase21_canonical_not_a_literal.py` |
| 31 | "Go source files in `server/` reference `ai/` only in comments — no runtime breakage." | The draft over-corrected to claim `//go:embed ai/...` directives exist — they **do not** (verified 2026-06-20). The real Go `ai/` references are three distinct kinds, only one of which is runtime-load-bearing: (a) **a runtime path constant** — `server/internal/sec/sanitize.go` has `const defaultTRNormalizeSpecPath = "common/text/tr_normalize_spec.json"`; this breaks at runtime if the path moves; (b) **a parity-test source-path table** — `server/internal/sec/embedded_parity_test.go` lists canonical `common/security/*.yaml` / `swarm/sdk/schemas/*.json` paths that are `cp`-mirrored into `server/internal/sec/embedded/` (the Go `//go:embed` targets are the *local* `embedded/` copies, never `ai/`); the parity test fails if the canonical source path is stale; (c) **doc comments** in `main.go`, `envelope.go`, `config.go`, `sync_test.go`, `patterns.go`. §22.5 fixes (a) and (b) as load-bearing and sweeps (c). There is no `go:embed` directive to fix. | `test_22_0_go_runtime_path_const_updated.py`, `test_22_0_go_parity_table_source_paths_updated.py`, `test_22_0_go_server_builds_after_path_migration.py` |
| 32 | "Redis key prefixes in the enrichment feature store are pure strings — no `ai.` reference." | Phase 21 §21.16 defines Redis keys as `f"enrich:{league_id}:{fixture_id}:{plane_id}"` — these do not include module paths. However, the Phase 8 `maint.*` reactor identity keys, the Phase 17 patcher leader-lease key (`patcher:leader`), and the Phase 21 circuit-breaker state keys (`enrich:circuit:{plane}`) are all path-independent. No Redis key migration is required. This wrong assumption is **pre-emptively retired** to confirm the analysis was done and no key migration is silently overlooked. | `test_22_0_redis_keys_have_no_ai_path_components.py` |
| 33 | "The `pyproject.toml` `known_first_party` list is updated in §22.5 with the new root packages — no impact before then." | `pyproject.toml` `known_first_party = ["ai", "common", "datasource", "swarm", ...]` is used by `isort` on every developer's machine and in CI to classify imports. A package that was `known_first_party` as `ai.scraper` but is not yet listed as `scraper` will be classified as a third-party import by isort, producing spurious lint failures for every developer working on the migrated package. The `known_first_party` list must be updated **in the same commit** as each package's migration, not in a batch at §22.5. | `test_22_0_isort_known_first_party_updated_per_package_commit.py`, `test_22_0_no_third_party_isort_classification_after_move.py` |
| 34 | "`swarm/` and root `swarm/` merge cleanly — `swarm/` is just an agents/ folder." | Root `swarm/` has `agents/` and `tests/`. `swarm/` has `agents/`, `identity/`, `predictor/`, `sdk/`, `source_watcher/`, `tests/`, and `bootstrap.py`. Both `agents/` sub-trees exist and must be merged without name collisions. `swarm/agents/` at root contains Phase 8 maintenance agents (`maint.scaler.v1`, `maint.backup.v1`, `maint.dlq.v1`, etc.) that are explicitly NOT owned by the AI pipeline patcher (per `CLAUDE.md`). Any agent module name collision between the two trees requires explicit renaming with a 90-day alias. A pre-merge collision scan is mandatory. | `test_22_0_swarm_agents_no_name_collision_before_merge.py`, `test_22_0_maint_agents_survive_swarm_merge.py` |
| 35 | "Migration telemetry is a nice-to-have — the burn-in dashboard is sufficient." | The 30-day burn-in (§22.13) requires tracking five counters daily. Without a dedicated `make phase22.burn-in.status` target that queries these from Redis / the Phase 14 dashboard, the burn-in is tracked manually via ad-hoc log searches — a failure mode that has caused missed signals in Phase 18's burn-in. `make phase22.burn-in.status` must be implemented before the burn-in starts, not after. The five counters (isolation regressions, `ai.` import errors, resurrections, metric-name violations, rollback invocations) each have a Redis key written by the CI post-merge hook; the status command reads them. | `test_22_0_burn_in_status_command_reads_five_counters.py`, `test_22_0_burn_in_status_implemented_before_burn_in_starts.py` |
| 36 | "The authoritative copy for every overlapping `common/` sub-package is the Phase-18 root version." | The shim direction is **heterogeneous** and the draft's blanket \"root wins\" rule is wrong (verified 2026-06-20): `common/bus/` (2 files) and `common/config/` (1 file) are **shims that re-export from `ai.common.*`** — for those the *authoritative implementation lives in `ai/`* and the move *flips the shim* (real code moves `common/bus` → `common/bus`, the shim is deleted). `common/db/`, `common/isolation/`, `common/observability/`, `common/security/` are **real root implementations** (root authoritative; `ai/` additions merged in). Applying \"root authoritative\" to `bus`/`config` would discard the real implementation and leave a shim pointing at a deleted tree. The per-sub-package direction must be declared explicitly in `docs/decisions/phase22/common_merge.md` and machine-checked. | `test_22_0_common_bus_and_config_are_ai_authoritative_shims.py`, `test_22_0_common_db_isolation_obs_sec_are_root_authoritative.py` |
| 37 | "Per-component lockfiles and SBOMs are plain files that `git mv` handles." | `swarm/` and `datasource/` each carry a hash-pinned `requirements.lock` + `sbom.spdx.json` (Phase 18 §18.10). Their content encodes the component path and dependency provenance; moving them verbatim leaves the SBOM `documentNamespace` / file-path entries pointing at `ai/`, and the Phase 18 §18.10 lockfile-review + SBOM-diff gate fails. Each must be **regenerated** for the new root path (`make lockfiles.regen` + `make sbom.gen`) and re-pass the gate in the **same commit** as the package move — not carried over byte-for-byte. The codemod (which only touches `*.py`) never sees them, so a separate step owns them. | `test_22_0_per_component_lockfile_regenerated_for_new_path.py`, `test_22_0_per_component_sbom_passes_review_after_move.py` |
| 38 | "Phase 17 patcher cassettes and bundles exist at Phase 22 time and must be migrated." | Phase 17 (Scraper-Patcher) ships **after** Phase 22 (it is unblocked *by* this phase). At execution time `datasource/patcher`, `datasource/gitops`, `xops/patcher/cassettes/`, and `feeds/ops/bundles/` **do not exist** (verified 2026-06-20 — only `xops/patcher/` is present, empty of cassettes/bundles). The cassette/bundle migration (§22.12) must therefore be **conditional**: migrate-if-present, assert-empty-if-absent. An unconditional `make patcher.cassette.migrate` that globs a non-existent directory either no-ops silently (masking a real future need) or errors (blocking the phase). The step must branch on artefact existence and record which branch it took. | `test_22_0_patcher_artifact_migration_is_conditional.py`, `test_22_0_patcher_migration_asserts_empty_when_phase17_unshipped.py` |
| 39 | "There is a single `enrichment` chart component to bump to `1.0.0`." | `xops/versioning/chart.json` has **no `enrichment` key** (verified 2026-06-20). Enrichment is four plane-scoped components — `enrichment_roster`, `enrichment_health`, `enrichment_officials`, `enrichment_environment` (seeded `0.1.0` in Phase 21 §21.0) — plus the path-move of `datasource/enrichment/` → `enrichment/` which is governed by **no single chart key**. §22.10's \"`enrichment` 1.0.0 bump\" references a key that does not exist; the bump list must name the real keys (and the `datasource_patcher` / `datasource_gitops` keys belong to the unshipped Phase 17, so their `1.0.0` bumps defer to that phase). | `test_22_0_no_single_enrichment_chart_key.py`, `test_22_0_version_bump_list_matches_real_chart_keys.py` |
| 40 | "`git mv` is enough — leftover bytecode is harmless." | `git mv ai/<pkg> <pkg>` does not move `__pycache__/`, but a developer's (or CI runner's) working tree often retains stale `ai/**/__pycache__/*.pyc`. Python's finder can import the stale compiled module under the *old* `ai.<pkg>` name during the transition window, masking import errors and producing non-deterministic test results that pass locally and fail on a fresh clone. Every move commit purges `__pycache__` before the test gate and CI sets `PYTHONDONTWRITEBYTECODE=1`; the fresh-clone DoD smoke (§22.13) is the authoritative check. | `test_22_0_no_stale_pycache_after_move.py`, `test_22_0_fresh_clone_has_no_orphan_pyc.py` |
| 41 | "There are two conftest.py files to reconcile (root + `ai/tests/`)." | There are **six** (verified 2026-06-20): `./conftest.py`, `common/tests/conftest.py`, `ai/tests/conftest.py`, `swarm/agents/tests/conftest.py`, `swarm/agents/maint/tests/conftest.py`, `xops/backup/tests/conftest.py`. The two nested `swarm/agents/**` conftests move during the **swarm** merge (§22.3c), not the tests merge; `common/tests/conftest.py` and `xops/backup/tests/conftest.py` already live at root and must be left untouched. Reconciling only the two the draft saw leaves the swarm-test conftests with stale `sys.path`/marker logic and risks duplicate-marker warnings across four registration sites. The full inventory drives the consolidation. | `test_22_0_all_six_conftests_inventoried.py`, `test_22_0_swarm_nested_conftests_reconciled_in_22_3c.py` |
| 42 | "`betting_markets.json` and `entitlements.yaml` are plain config files that move verbatim." | `common/betting_markets.json` is read via a config key and is consumed by both the Python pipeline and (indirectly) the Go entitlement edge; moving it to `data/betting_markets.json` is a **data-contract change** requiring `cfg.betting_markets_path` to update atomically. `entitlements.yaml` exists in **two** places — `common/entitlements.yaml` and `xops/monetization/entitlements.yaml` (Phase 20 canonical) — so the move is a **de-duplication**, not a relocation: the `common/` copy is reconciled against (and likely deleted in favour of) the `xops/monetization/` canonical, with a diff-audit committed. Treating either as a verbatim `git mv` forks the data contract. | `test_22_0_betting_markets_path_is_config_driven_after_move.py`, `test_22_0_entitlements_deduplicated_against_monetization_canonical.py` |
| 43 | "`git mv` plus the no-duplicate-symbols test guarantees nothing is lost or overwritten." | `git mv` into an existing path either errors (good) or, with `-f`, **silently clobbers** — and the no-duplicate-symbols test proves the *opposite* property (no collision), not preservation. Neither proves that every source file reached a destination, nor that a discarded copy's unique content was salvaged. Phase 22 adds a **file-accountability manifest** (`docs/tracking/phase22_file_accountability.json`, built by `make phase22.inventory`, excluding `__pycache__`): every `ai/` source path maps to exactly one destination action (`move` / `merge:<decision-doc>` / `delete:<reason>`); post-migration a gate asserts (a) every source path is accounted for, (b) no destination received two sources without a recorded merge decision, and (c) no `git mv` was run with `-f`. | `test_22_0_every_source_file_accounted_for.py`, `test_22_0_no_destination_has_two_unrecorded_sources.py`, `test_22_0_no_forced_git_mv_in_migration.py` |
| 44 | "Choosing an authoritative copy for a colliding file is safe because CI would catch a lost symbol." | CI green proves only that **tested** code still imports; a public symbol that existed in the discarded copy but has no test silently vanishes and ships. Phase 22 adds a **symbol-union preservation gate**: for every merged module, collect the public surface (`__all__` if present, else module-level names not starting with `_`) of *both* pre-move copies; assert the post-merge module's public surface is a **superset** of their union, except names explicitly listed in `docs/decisions/phase22/dropped_symbols.md` — and each dropped name must carry a `vulture`/grep-backed "no live caller" proof in the same decision row. This is the actual "nothing lost" guarantee for the heterogeneous merges. | `test_22_0_merge_preserves_symbol_union.py`, `test_22_0_dropped_symbol_has_no_live_caller_proof.py` |
| 45 | "A verbatim package move is byte-for-byte safe by definition." | A "verbatim" move still runs the codemod (import rewrites), and a careless tool can reflow, re-encode line endings, or strip a trailing newline — changing bytes the migration never intended to touch. Phase 22 emits a **content-hash manifest** (`make phase22.inventory` records each file's pre-move blob SHA) and a post-move gate asserts every moved file is byte-identical **except** the specific lines the codemod was authorised to rewrite (verified by re-deriving the rewrite and diffing). A file whose hash changed without a corresponding authorised rewrite fails the gate. | `test_22_0_verbatim_move_preserves_blob_hash.py`, `test_22_0_only_authorised_lines_changed_in_moved_file.py` |

`xops/lint/phase22_ledger.py` enforces: every retired-row PR ships a green proof test in the same diff; the ledger is dense (no row gaps) and append-only (no row deletion). **45 rows total.**

---

### 22.1 — Pre-flight inventory & safety gates
...
and install the automated codemod engine. **This sub-phase ships infrastructure
only — zero package moves.**

**Complete directory move plan (canonical).** The `Non-`ai/` callers` column
is **not hand-counted** — exact per-package caller sets come from
`make phase22.import-report` (the 55 non-`ai/` importers are distributed
**44 `xops/` + 10 `common/` + 1 `docs/`**, no `swarm/`/`server/`). The values
below mark only *whether* a package has non-`ai/` callers; the report is
authoritative for *which*.

| Source | Destination | Action | Overlap? | Non-`ai/` callers? |
|---|---|---|---|---|
| `scraper/` | `scraper/` | Move | No | per import-report (`xops/`, `common/`) |
| `model/` | `model/` | Move | No | per import-report |
| `nlp/` | `nlp/` | Move | No | per import-report |
| `orchestrator/` | `orchestrator/` | Move | No | per import-report |
| `pipeline/` | `pipeline/` | Move | No | per import-report |
| `proofreader/` | `proofreader/` | Move | No | per import-report |
| `qid/` | `qid/` | Move | No | per import-report |
| `tqu/` | `tqu/` | Move | No | per import-report |
| `trc/` | `trc/` | Move | No | per import-report |
| `ai/backtest/` | `backtest/` | Move | No | per import-report |
| `ai/tests/` | `tests/` | **Merge** (§22.3b) | Partial — root `tests/` does **not** exist; created by this move | — |
| `datasource/enrichment/` | `enrichment/` | Move | No | per import-report |
| `datasource/t3_resource_manager.py` | `enrichment/t3_resource_manager.py` | Move | No | per import-report |
| `datasource/requirements.lock` | `enrichment/requirements.lock` | **Regenerate** (ledger #37) | No | — |
| `datasource/sbom.spdx.json` | `enrichment/sbom.spdx.json` | **Regenerate** (ledger #37) | No | — |
| `datasource/quarantine.py` (root stub) | `scraper/quarantine.py` | Move | No | per import-report |
| `datasource/` (root stub, now empty) | (deleted) | Delete | — | — |
| `ai/main.py` | `main.py` | Move | No | — |
| `ai/scheduler.py` | `scheduler.py` | Move | No | — |
| `ai/data_showcase.py` | `data_showcase.py` | Move | No | — |
| `ai/requirements.txt` | `requirements.txt` | pip-compile merge | Partial — no root counterpart | — |
| `ai/requirements-dev.txt` | `requirements-dev.txt` | pip-compile merge | Partial | — |
| `ai/Dockerfile` | `Dockerfile` | Merge/replace | No — only `server/` has another Dockerfile | — |
| `common/` | `common/` | **Merge** (§22.3a) | Yes — **5** overlapping sub-pkgs (`bus`,`db`,`isolation`,`observability`,`security`) + 2 top files; `feeds`/`api` are `ai`-only move-ins | per import-report (`xops/`, `common/`) |
| `common/betting_markets.json` | `data/betting_markets.json` | **Data-contract move** (ledger #42) | No | config key |
| `common/entitlements.yaml` | (de-dup vs `xops/monetization/entitlements.yaml`) | **De-duplicate** (ledger #42) | Yes — Phase 20 canonical exists | — |
| `swarm/` | `swarm/` | **Merge** (§22.3c) | Yes — `agents/` + `tests/` + 2 nested conftests | per import-report |
| `swarm/requirements.lock` | `swarm/requirements.lock` | **Regenerate** (ledger #37) | Yes — root `swarm/` is a Phase 18 component | — |
| `swarm/sbom.spdx.json` | `swarm/sbom.spdx.json` | **Regenerate** (ledger #37) | Yes | — |
| `ai/docs/` | `docs/ai_pipeline/` | **Merge** (§22.3d) | Partial | — |
| `ai/reports/` | `docs/reports/ai_pipeline/` | Move | No | — |

Non-`ai/` files that import `ai.*` (ledger #28) are co-updated in the **same
commit** as the package they reference. The §22.1 inventory must enumerate
every such file per-package, not just the `ai/` files. **`server/` is Go** — its
`ai/` references (one runtime path constant + a parity-test source-path table +
doc comments, ledger #31) are handled by §22.5, not the Python codemod.

**Pre-flight checklist (ALL must be green before the first file move):**

- [ ] `PYTHONPATH=. make test` is green on the current `ai/`-based transitional
  layout. Any pre-existing failure is fixed before the migration starts.
  No migration commit may be made against a red test baseline.
- [ ] `make isolation.shims-only` exits non-zero (confirming `ai/` still contains
  real implementation — the gate is designed to fail until §22.4 is complete).
  If it exits 0 unexpectedly, the migration was partially started without this
  phase — pause and audit.
- [ ] Phase 18 §18.3 pre-conditions recorded in
  `docs/tracking/phase18_shim_deletion_acks.md`: zero `ai.*-shim`
  `DeprecationWarning` in CI for ≥ 14 d across ≥ 50 pipelines; zero production
  runtime shim hits (`make shim.runtime.report`); CODEOWNERS ACKs for all
  four components.
- [ ] Phase 19 §19.1 fourth signal: `make patcher.bundle.scan-ai-refs` exits 0.
- [ ] `make phase22.inventory` exits 0 and produces
  `docs/tracking/phase22_move_plan.yaml`.
- [ ] `make phase22.codemod.check` exits 0 — `libcst ≥ 1.1.0` and
  `pydeps ≥ 1.12.0` are importable in the dev venv.
- [ ] `docs/runbooks/phase22_rollback.md` committed and covers all 14
  migration steps with per-step rollback commands.
- [ ] `make phase22.cycle-check` runs `pydeps` on the current `ai/` tree and
  produces `docs/tracking/phase22_pre_migration_cycle_graph.json` as a baseline;
  any detected cycle already present must be resolved before any package moves.
- [ ] `make phase22.import-report` emits per-file counts of `from ai.`
  references; files with > 50 import sites are flagged for manual review. A
  per-package non-`ai/` caller list is emitted alongside.
- [ ] `make phase22.dead-code` runs `vulture ai/` at 80% confidence and commits
  `docs/tracking/phase22_dead_code_candidates.md`. Confirmed dead symbols are
  removed in dedicated cleanup PRs **before** the package moves.
- [ ] `make phase22.burn-in.status` target implemented and exits 0 even with
  zeroed counters — the status infrastructure must exist before the burn-in
  starts (ledger #35).
- [ ] Swarm agents collision scan: `make phase22.swarm-collision-check` enumerates
  module names in `swarm/agents/` vs `swarm/agents/` and asserts zero
  collisions; any collision is resolved (rename + 90-day alias) before §22.3c.
- [ ] Go source scan: `make phase22.go-path-check` scans all `*.go` files for
  `ai/` path **string literals** (the runtime const `defaultTRNormalizeSpecPath`,
  the `embedded_parity_test.go` source-path table, and doc comments — there are
  **no `//go:embed ai/`** directives, ledger #31); every hit is documented in
  `docs/tracking/phase22_go_path_hits.txt`, tagged load-bearing vs comment, for §22.5.
- [ ] Conftest inventory: `make phase22.conftest-inventory` lists all **6**
  `conftest.py` files (ledger #41), classifies each as *move* (`ai/tests`,
  `swarm/agents/tests`, `swarm/agents/maint/tests`), *root-merge target*
  (`./conftest.py`), or *untouched* (`common/tests`, `xops/backup/tests`), and
  records the marker-registration and `sys.path`/`sys.modules` surface of each.
- [ ] Shim-direction decision: `docs/decisions/phase22/common_merge.md` committed
  declaring, per overlapping `common/` sub-package, whether the authoritative copy
  is the `ai/` real implementation (`bus`, `config` — flip the shim) or the root
  real implementation (`db`, `isolation`, `observability`, `security`) — ledger #36.
- [ ] Phase 17 artefact-absence probe: `make phase22.patcher-artifact-probe`
  records whether `xops/patcher/cassettes/` and `feeds/ops/bundles/` exist
  (ledger #38). If absent (expected — Phase 17 unshipped), §22.12 runs in
  assert-empty mode; if present, it runs in migrate mode. The probe result is
  written to `docs/tracking/phase22_patcher_artifact_state.txt`.
- [ ] Per-component lockfile/SBOM baseline: `make phase22.lockfile-baseline`
  records the pre-move `requirements.lock` + `sbom.spdx.json` digests for
  `swarm/` and `datasource/` so §22.4 can prove regeneration (not verbatim
  carry-over) per ledger #37.
- [ ] **Nothing-lost manifest (ledger #43–#45):** `make phase22.inventory` emits
  `docs/tracking/phase22_file_accountability.json` mapping **every** `ai/` source
  file (excluding `__pycache__`) to exactly one destination action
  (`move` / `merge:<decision-doc>` / `delete:<reason>`) **and** records each
  file's pre-move blob SHA. `make phase22.collision-map` cross-checks every
  destination for two-source collisions and emits
  `docs/decisions/phase22/common_merge.md` stubs for each colliding+differing
...
- [x] Proof tests:
  ...
  - `test_22_1_codemod_engine_version_pinned.py` — `libcst >= 1.1.0` and `pydeps >= 1.12.0`.
  - `test_22_1_rollback_runbook_committed_and_covers_all_steps.py` — runbook exists and contains a section for each of the 14 migration steps.
  - `test_22_1_cycle_baseline_committed.py` — `phase22_pre_migration_cycle_graph.json` exists.
  - `test_22_1_swarm_collision_check_is_clean.py` — no agent module name collisions before §22.3c.
  - `test_22_1_go_path_check_produces_classified_hit_list.py` — scanner runs and tags each `ai/` Go reference load-bearing vs comment (no `go:embed` hits expected).
  - `test_22_1_conftest_inventory_lists_all_six.py` — all six conftests inventoried and classified (ledger #41).
  - `test_22_1_common_merge_decision_doc_committed.py` — per-sub-package authoritative-copy direction declared (ledger #36).
  - `test_22_1_patcher_artifact_probe_records_state.py` — probe writes present/absent for cassettes + bundles (ledger #38).
  - `test_22_1_lockfile_baseline_recorded.py` — pre-move lock/SBOM digests captured for `swarm`, `datasource` (ledger #37).
  - `test_22_1_file_accountability_manifest_complete.py` — every `ai/` source file (minus `__pycache__`) maps to exactly one destination action (ledger #43).
  - `test_22_1_collision_map_has_decision_stub_per_colliding_file.py` — every colliding+differing file has a `common_merge.md` decision stub (ledger #27/#44).
  - `test_22_1_blob_sha_baseline_recorded.py` — pre-move blob SHA captured for every file (ledger #45).
  - `test_22_1_burn_in_status_exits_zero_before_migration.py` — `make phase22.burn-in.status` exits 0 with zeroed counters.

---

### 22.2 — Automated codemod engine

All import rewrites are performed by a `libcst`-based codemod engine, not
...
- [x] **`xops/makefile/phase22.py`** ships the full command surface:
  ...
  - `make phase22.cycle-check` — `pydeps`-based cycle analysis pre/post move; fails on any new cycle.
  - `make phase22.import-report` — per-file import-site count report.
  - `make phase22.dead-code` — `vulture ai/` scan.
  - `make phase22.go-path-check` — scan Go source for `ai/` path string literals (runtime const + parity-test table + comments; **no `//go:embed ai/`**, ledger #31).
  - `make phase22.swarm-collision-check` — check swarm agents for name collisions.
  - `make phase22.conftest-inventory` — list + classify all six `conftest.py` files (ledger #41).
  - `make phase22.patcher-artifact-probe` — record presence/absence of Phase 17 cassettes + bundles (ledger #38).
  - `make phase22.lockfile-baseline` / `make phase22.lockfile-migrate PACKAGE=<pkg>` — capture pre-move lock/SBOM digests, then regenerate for the new path (ledger #37).
  - `make phase22.collision-map` — cross-check every destination for two-source collisions; emit `common_merge.md` decision stubs (ledger #27/#43/#44).
  - `make phase22.verify-no-loss PACKAGE=<pkg>` — assert file-accountability + blob-SHA preservation + no `git mv -f` for a package (ledger #43–#45).
  - `make phase22.rollback STEP=<N>` — reverse-order revert + isolation check + test run.
  - `make phase22.burn-in.status` — five burn-in counter report.
...
- [x] **`xops/codemod/phase22_rewriter.py`** (`libcst.CSTTransformer` subclass)
  ...
  3. Quoted type annotations: `"ai.<pkg>.<Class>"` → `"<pkg>.<Class>"` — walks `SimpleString`, `ConcatenatedString`, and `FormattedString` nodes
  4. `TYPE_CHECKING` block imports (same as patterns 1/2, including `typing_extensions.TYPE_CHECKING`)
  ...
  7. **Generated file provenance headers**: `# negelir-generated-from: ai/<path>@<sha256>` → `# negelir-generated-from: <path>@<sha256>` (only in the **3** files that actually carry the header — discovered by `make phase22.inventory`, not the phantom `common/schemas/_generated/` or `swarm/clients/` of ledger #14).
...
- [x] Proof tests:
  ...
  - `test_22_2_codemod_does_not_rewrite_sql_and_log_strings.py` — strings containing `"ai."` as part of non-import context are left unchanged.
  - `test_22_2_codemod_dry_run_writes_nothing.py` — file mtimes unchanged after dry-run.
  - `test_22_2_codemod_include_non_ai_callers_rewrites_all_callers.py` — non-`ai/` caller files are rewritten when the flag is set.
  - `test_22_2_codemod_rollback_restores_orig_file.py` — sidecar exists before rewrite; restore is byte-identical to pre-rewrite.
  - `test_22_2_codemod_handles_type_checking_block.py` — `TYPE_CHECKING` block imports correctly rewritten.
  - `test_22_2_codemod_rewrites_pydantic_forward_ref.py` — `model_rebuild()` call updated.
  - `test_22_2_codemod_rewrites_annotation_string_literals.py` — quoted type names in `Annotated[...]` and `TypeVar` bounds updated.
  - `test_22_2_codemod_rewrites_generated_provenance_headers.py` — `# negelir-generated-from: ai/...` header lines updated.
  - `test_22_2_codemod_rejects_non_py_extensions.py` — engine refuses to edit `.lock`/`.json`/`.yaml`/`.go`/`.pyc` files (scope boundary, ledger #37/#42).
  - `test_22_2_codemod_full_package_completes_within_60s.py` — `@pytest.mark.perf` timing gate.

---

### 22.3 — Merge conflicting folders

The four folders requiring conflict-aware merging are processed first, in
dependency order, before any non-conflicting package moves. Each is a
**separate commit** gated by a full `PYTHONPATH=. make test` run.

#### 22.3a — Merge `common/` → `common/`

Root `common/` is the Phase 18 governance-layer shared library. `common/` is
the larger AI-pipeline shared library. **Exactly five** sub-packages overlap
(`bus/`, `db/`, `isolation/`, `observability/`, `security/`); `common/feeds/`
and `common/api/` have **no root counterpart** (they are move-ins, not
merges). Two top-level files also conflict: `common/logger.py` vs
`common/logger.py`, and `common/international_tournament_profiles.py` vs
`common/international_tournament_profiles.py`.

> **⚠️ The authoritative copy is heterogeneous (ledger #36).** Two of the five
> overlapping root sub-packages are **shims that re-export from `ai.common.*`** —
> verified 2026-06-20: `common/bus/` (`__init__.py` + `publisher.py`) and
> `common/config/__init__.py`. For those the **real implementation is in `ai/`**
> and the move *flips the shim* (real code moves to root, the shim is deleted).
> The other three (`db`, `isolation`, `observability`, `security`) are **real
> root implementations** and the `ai/` copy contributes only net-additional
> symbols. Applying a single "root wins" rule to all five — as the draft did —
> would delete the real `bus`/`config` implementation and leave a shim pointing
> at a deleted tree.

*Per-sub-package authoritative-copy decision (committed to
`docs/decisions/phase22/common_merge.md` before any file moves):*

| Overlap | Today | Authoritative copy | Move action |
|---|---|---|---|
| `common/bus/` | root is a **shim** → `ai.common.bus` | **`common/bus/`** | move real impl to `common/bus/`, delete shim, codemod its importers |
| `common/config/` | `__init__.py` is a **shim** → `ai.common.config` | **`common/config.py`** | move to `common/config/ai_pipeline.py`, repoint `__init__.py` |
| `common/db/` | real root impl | **root** | merge `ai/` net-additional symbols only |
| `common/isolation/` | real root impl (Phase 18 `go_check.go` + snapshot) | **root** | 4 root-only files preserved; 4 colliding+differing files symbol-union merged onto root base (ledger #27) |
| `common/observability/` | real root impl | **root** | merge `ai/` net-additional symbols only |
| `common/security/` | real root impl | **root** | merge `ai/` net-additional symbols only |

*Conflict-resolution protocol — applied to every overlapping file and sub-package:*
1. Diff the two copies and apply the **direction declared in the table above**;
   never assume a uniform winner. For a root-authoritative sub-package, only
   symbols the `ai/` copy uniquely contributes are merged in. For an
   `ai/`-authoritative shim, the real implementation moves to root and the shim
   is deleted (not aliased — a shim pointing at a deleted tree is the bug this
   row prevents).
2. **The four root-only isolation files survive untouched** — `go_check.go`,
   `graph_builder.py`, `forbidden_deps.yaml`, `import_graph.snapshot.json` exist
   only in root `common/isolation/` and carry the Phase 18 gate itself. The four
   files that collide and **differ** (`check.py`, `policy.py`, `dpa_validator.py`,
   `__init__.py`) each get a **per-file diff-audit + symbol-union merge** (root
   base, `ai/`-unique public symbols folded in), recorded in
   `docs/decisions/phase22/common_merge.md` — **not** a blind discard of either
   side (ledger #27).
3. For `common/logger.py` vs `common/logger.py` (both differ): the `ai/`
   version is richer; it becomes `common/logger.py` after a symbol-union check
   (ledger #44) confirms no public symbol of the root copy is lost. The decision
   is documented in `docs/decisions/phase22/common_merge.md`.
4. For `common/international_tournament_profiles.py` vs
   `common/international_tournament_profiles.py` (both differ — verified
   2026-06-20, **previously unresolved**): diff-audit both, choose the
   authoritative copy, symbol-union the loser's unique public names, record the
   decision. The top-level `__init__.py` collision is resolved the same way (its
   `__all__` is the union of both).
5. Where a 90-day alias is genuinely needed (a *consumer* still importing the
   old path that cannot be co-updated in the same commit), the alias is a
   `DeprecationWarning` re-export logged in `xops/lifecycle/deprecation_calendar.yaml`.
   Shims that re-export from a *deleted* tree are forbidden — alias only ever
   points at a live root path.

**Files unique to `common/` → move verbatim:**
- `common/calibration_profiles/` → `common/calibration_profiles/`
- `common/calibration_profile_loader.py` → `common/calibration_profile_loader.py`
- `common/catalog_api_models.py` → `common/catalog_api_models.py`
- `common/catalog_cache.py` → `common/catalog_cache.py`
- `common/competition_fsm.py` → `common/competition_fsm.py`
- `common/competition_multistage.py` → `common/competition_multistage.py`
- `common/config.py` → `common/config/ai_pipeline.py`; `common/config/__init__.py` updated to import from `common.config.ai_pipeline` (removing the current transitional `from ai.common.config import ...` import — ledger #26)
- `common/constants.py` → `common/constants.py` (with `CURRENT_SEASON` replaced by a config read per §22.6 — ledger #29)
- `common/defaults.yaml` → `common/config/defaults.yaml`
- `common/fixture_state.py` → `common/fixture_state.py`
- `common/fixture_timezone_resolver.py` → `common/fixture_timezone_resolver.py`
- `common/fixture_validator.py` → `common/fixture_validator.py`
- `common/league_catalog.yaml` → `common/catalog/league_catalog.yaml`
- `common/league_catalog_loader.py` → `common/catalog/league_catalog_loader.py`
- `common/league_config.py` → `common/league_config.py`
- `common/leagues/` → `common/leagues/`
- `common/locale_loader.py` → `common/locale_loader.py`
- `common/locale_tr.yaml` → `common/locale_tr.yaml`
- `common/nlp/` → `common/nlp_vocab/` (**renamed** to prevent collision with the root `nlp/` package)
- `common/numerics.py` → `common/numerics.py`
- `common/parity.py` → `common/parity.py`
- `common/profile_drift_guard.py` → `common/profile_drift_guard.py`
- `common/season.py` → `common/season.py`
- `common/season_calendar.py` → `common/season_calendar.py`
- `common/seed.py` → `common/seed.py`
- `common/standings_accumulator.py` → `common/standings_accumulator.py`
- `common/telemetry.py` → `common/telemetry.py` (after metric-name freeze from §22.9)
- `common/text/` → `common/text/`
- `common/schemas/` → `common/schemas/` (requires new sub-package charter entry + triple CODEOWNERS ACK per Phase 18 §18.13 before the move — ledger #7)
- `common/api/` → merge into `common/api/` (diff before touching)
- `common/betting_markets.json` → `data/betting_markets.json` (config key `cfg.betting_markets_path` updated)
- `common/entitlements.yaml` → deduplicated against `xops/monetization/entitlements.yaml`

**Overlapping sub-packages — authoritative-copy selection (heterogeneous, ledger #36):**
- `common/bus/` vs `common/bus/`: **`ai/` is authoritative** (root `common/bus/__init__.py` + `publisher.py` are shims re-exporting `ai.common.bus`). The real implementation moves to `common/bus/`; the shim is **deleted**; every importer (including the 10 `common/` and 44 `xops/` non-`ai/` callers) is repointed by the codemod in the same commit.
- `common/config.py` vs `common/config/`: **`ai/` is authoritative** (root `common/config/__init__.py` is a shim with a `sys.path.insert`). The real config moves to `common/config/ai_pipeline.py`; `__init__.py` is rewritten to `from common.config.ai_pipeline import Config, cfg` with the `sys.path` hack removed.
- `common/db/` vs `common/db/`: **root is authoritative**; only `ai/`-unique symbols merged in.
- `common/isolation/` vs `common/isolation/`: **heterogeneous, per-file** (ledger #27). Four root-only files (`go_check.go`, `graph_builder.py`, `forbidden_deps.yaml`, `import_graph.snapshot.json`) are preserved untouched; four colliding+differing files (`check.py`, `policy.py`, `dpa_validator.py`, `__init__.py`) are symbol-union merged onto the root base. There are **no net-new `ai/` source files** here — the earlier "only `dpa_validator.py`" claim was wrong.
- `common/observability/` vs `common/observability/`: **root is authoritative**; the colliding+differing `metrics.py`, `alerts.py`, `__init__.py` get a per-file symbol-union merge (not a discard); `ai/`-unique modules merged. ✓ **Completed Phase 22.3.6**
- `common/security/` vs `common/security/`: **root is authoritative**; the colliding+differing `input_sanitiser.py` gets a per-file symbol-union merge; `ai/`-unique modules merged. (Note: `server/internal/sec/` mirrors `common/security/*.yaml` via the §22.5 parity table — the canonical-source path in `embedded_parity_test.go` updates to `common/security/...` in the same window.)
- `common/feeds/`, `common/api/`: **`ai`-only — no overlap.** Pure move-ins to `common/feeds/`, `common/api/`. (`common/feeds/` is governed by the `common_feeds` chart key.)

`common/SUBPACKAGE_CHARTER.md` extended with entries for: `common.schemas`, `common.calibration_profiles`, `common.catalog`, `common.text`, `common.nlp_vocab`, `common.leagues`, `common.numerics`, `common.parity`, `common.season`, `common.season_calendar`, `common.fixture_state`, `common.fixture_timezone_resolver`, `common.fixture_validator` (all require triple CODEOWNERS ACK before move).

- [ ] No symbol defined in more than one module under `common/`.
- [ ] **Nothing-lost gates green for the merge (ledger #43–#45):**
  - File-accountability: every `common/**` source file mapped to exactly one
    destination action; no destination received two sources without a recorded
    decision; no `git mv -f` used.
  - Symbol-union: every merged module's public surface ⊇ union of both source
    copies' public surfaces, minus the `dropped_symbols.md` allow-list (each
    entry with a no-live-caller proof).
  - Content-hash: every verbatim-moved file byte-identical except authorised
    codemod rewrites.
...
- [ ] `data/betting_markets.json` present and `cfg.betting_markets_path` updated; old `common/betting_markets.json` gone (ledger #42).
- [ ] `common/entitlements.yaml` reconciled against `xops/monetization/entitlements.yaml`; diff-audit committed; no forked second copy (ledger #42).
- [ ] Proof tests: `test_22_3a_common_merge_no_duplicate_symbols.py`, `test_22_3a_merge_preserves_symbol_union.py`, `test_22_3a_file_accountability_complete.py`, `test_22_3a_no_forced_git_mv.py`, `test_22_3a_isolation_root_only_files_preserved.py`, `test_22_3a_intl_tournament_profiles_conflict_resolved.py`, `test_22_3a_top_level_init_union_resolved.py`, `test_22_3a_common_schemas_subpackage_charter.py`, `test_22_3a_ai_common_nlp_becomes_nlp_vocab.py`, `test_22_3a_logger_conflict_resolved_single_authoritative.py`, `test_22_3a_config_init_no_longer_imports_ai.py`, `test_22_3a_bus_shim_flipped_no_reexport_from_ai.py`, `test_22_3a_no_common_module_reexports_from_ai_tree.py`, `test_22_3a_go_check_survives_merge.py`, `test_22_3a_dpa_validator_symbol_union_merged.py`, `test_22_3a_feeds_and_api_are_pure_move_ins.py`, `test_22_3a_betting_markets_config_key_updated.py`, `test_22_3a_entitlements_deduplicated.py`.

#### 22.3b — Merge `ai/tests/` → `tests/`

There is no root-level `tests/` directory today. This step creates it by
moving `ai/tests/` and reconciling with the root `conftest.py`. **Six conftests
exist** (ledger #41); this step touches only the test-tree ones, leaving the
already-rooted `common/tests/conftest.py` and `xops/backup/tests/conftest.py`
untouched, and deferring the two nested `swarm/agents/**/conftest.py` to the
swarm merge (§22.3c).
...
- [x] Conftest reconciliation map (from `make phase22.conftest-inventory`):
  ...
  `swarm/agents/tests/conftest.py` + `swarm/agents/maint/tests/conftest.py`
...
#### 22.3c — Merge `swarm/` → `swarm/`

Root `swarm/` has `agents/` (Phase 8 maintenance agents) and `tests/`.
`swarm/` has the full swarm AI implementation.

Merge rules:
- `swarm/agents/` → merged into `swarm/agents/`. The Phase 8 maint agents (`maint.scaler.v1`, `maint.backup.v1`, `maint.dlq.v1`, `maint.schema.v1`, `maint.sec.v1`) in root `swarm/agents/` are **CODEOWNERS-protected from patcher scope** per `CLAUDE.md`. Any naming collision with `swarm/agents/` requires an explicit rename decision, documented in `docs/decisions/phase22/swarm_agent_collision_<name>.md`, and a 90-day alias.
- `swarm/agents/tests/conftest.py` + `swarm/agents/maint/tests/conftest.py` → moved with their `agents/` sub-trees; `sys.path`/`sys.modules` manipulation stripped and markers deduplicated against the root conftest (deferred from §22.3b, ledger #41).
- `swarm/identity/` → `swarm/identity/`
- `swarm/predictor/` → `swarm/predictor/`
- `swarm/sdk/` → `swarm/sdk/`
- `swarm/source_watcher/` → `swarm/source_watcher/`
- `swarm/tests/` → merged into `swarm/tests/` (no duplicate test file names)
- `swarm/bootstrap.py` → `swarm/bootstrap.py`
- `swarm/requirements.lock` + `swarm/sbom.spdx.json` → **regenerated** for the merged `swarm/` component (ledger #37), not carried over; re-pass the Phase 18 §18.10 lockfile-review gate in the same commit.

> **No `swarm/clients/` exists** (verified 2026-06-20) — the draft's
> `swarm/clients/server_internal/` generated-stub move is a phantom. The
> Go↔Python wire-contract parity (`server/internal/sec/` mirroring
> `swarm/sdk/schemas/*.json`) is a **Go-side** concern handled by the §22.5
...
- [x] All critical modules moved from `common/` to root `common/`: 19 modules (api, calibration_profile_loader, catalog_cache, competition_fsm, constants, feeds, fixture_state, fixture_timezone_resolver, league_catalog_loader, league_config, leagues, locale_loader, logger, numerics, profile_drift_guard, schemas, season, season_calendar, seed, standings_accumulator, telemetry, text).
...
- [x] All `ai/` directories (common, swarm, datasource) cleaned of .py files except __init__.py shims.
...
- [ ] All markdown files under `ai/docs/` absorbed into `docs/ai_pipeline/`.
- [ ] No root `docs/` file overwritten without explicit CODEOWNERS review.
- [ ] All internal cross-references in `ai/docs/` updated from `ai/docs/...` to `docs/ai_pipeline/...`.
...
determined by `make phase22.cycle-check`. Each package is a **separate commit**;
`PYTHONPATH=. make test` must be green before the next package moves.

**Dependency-ordered move sequence** (confirmed by `pydeps` analysis before
execution — the order below is the pre-computed canonical order; verify it
matches `phase22_pre_migration_cycle_graph.json` before proceeding):

1. `scraper/` — **first** (no AI-layer dependents at root; absorbs `datasource/quarantine.py` → `scraper/quarantine.py`; root `datasource/` stub directory deleted)
2. `model/` — (depends on `common/`, `scraper/`)
3. `nlp/` — (depends on `common/`)
4. `qid/` — (depends on `common/`, `nlp/`)
5. `tqu/` — (depends on `common/`, `nlp/`, `qid/`)
6. `trc/` — (depends on `common/`, `tqu/`)
7. `proofreader/` — (depends on `common/`, `model/`)
8. `pipeline/` — (depends on `common/`, `model/`, `scraper/`, `proofreader/`)
9. `orchestrator/` — (depends on `common/`, `pipeline/`)
10. `backtest/` — (depends on `common/`, `model/`, `scraper/`)
11. `enrichment/` — from `datasource/enrichment/` + `datasource/t3_resource_manager.py` (depends on `common/`, `scraper/`, `model/`); also regenerate `datasource/requirements.lock` + `sbom.spdx.json` for the new `enrichment/` path (ledger #37). **No single `enrichment` chart key exists** (ledger #39) — versioning is per the four `enrichment_*` plane keys, handled in §22.10.
12. Root entry-point files: `main.py`, `scheduler.py`, `data_showcase.py`
13. `ai/reports/` → `docs/reports/ai_pipeline/`

> **If `pydeps` reveals a different ordering**, the sequence above must be
> updated in `docs/tracking/phase22_move_plan.yaml` before any commit. The
> ROADMAP prose is informational; the YAML is authoritative.

**Per-package commit protocol (mandatory for every package):**

1. `make phase22.codemod PACKAGE=<pkg>` — rewrites import sites in `ai/` **and**
   all non-`ai/` callers of `<pkg>` identified in the inventory (`--include-non-ai-callers`).
2. `git mv ai/<pkg>/ <pkg>/` — move the directory.
3. `find . -name '__pycache__' -prune -exec rm -rf {} +` — purge stale bytecode
   that could shadow the moved module (ledger #40).
4. **If `<pkg>` carries a `requirements.lock` / `sbom.spdx.json`** (`swarm/`,
   `enrichment/`): `make phase22.lockfile-migrate PACKAGE=<pkg>` regenerates them
   for the new path and re-passes the Phase 18 §18.10 lockfile-review gate (ledger #37).
5. Update `pyproject.toml` `known_first_party` to include `<pkg>` and remove
   `ai.<pkg>` (ledger #33).
6. `make isolation.snapshot.refresh` — rebuild snapshot (mandatory per ledger #8).
7. `make isolation.check --full` — must exit 0.
8. `PYTHONPATH=. make test` — full suite green (fresh-bytecode run).
9. `make phase22.cycle-check` — no new cycles.
10. `make phase22.verify-no-loss PACKAGE=<pkg>` — assert (a) every source file of
    `<pkg>` is accounted for in the manifest, (b) every verbatim-moved file's
    blob SHA matches the baseline except authorised codemod rewrites, (c) no
    `git mv -f` was used (ledger #43–#45).
11. `git commit -m "phase22: move <pkg>/ to root"` — single atomic commit
    covering: moved files + rewritten import sites + regenerated lock/SBOM +
    `pyproject.toml` + snapshot. **PR stays under 150 changed files** (ledger #19).
...
### 22.5 — Build, CI, k8s, xops, and Go artefact migration

Every build and operations artefact must reference the new layout. Run after
§22.4 is fully green.

- [x] **Root `Dockerfile`** merges `ai/Dockerfile` (multi-stage build,
  `python:3.12-slim` base, digest-pinned per Phase 18 §18.10):
  - All `COPY ai/ .` → `COPY . .`
  - `.dockerignore` updated to exclude `xops/`, `docs/`, `data/`, `infra/`,
    `migrations/`, `server/`, `*.md`, `.git/`
  - All `ENV PYTHONPATH=ai` → `ENV PYTHONPATH=.`
  - `ai/Dockerfile` deleted after content is absorbed.
  - Post-migration image size verified within ±10% of pre-migration baseline
    (ledger #6).

- [x] **Go source files** — Phase 22 Go scan resolves all hits from `make phase22.go-path-check` (ledger #31; there are **no `//go:embed ai/`** directives):
  - **Load-bearing #1 — runtime path constant.** `server/internal/sec/sanitize.go`
    `const defaultTRNormalizeSpecPath = "common/text/tr_normalize_spec.json"`
    → `"common/text/tr_normalize_spec.json"`. Breaks Turkish input sanitisation
    at runtime if missed.
  - **Load-bearing #2 — parity-test source-path table.**
    `server/internal/sec/embedded_parity_test.go` lists canonical source paths
    (`common/security/injection_patterns.yaml`,
    `common/security/endpoint_costs.yaml`, `swarm/sdk/schemas/qa.request.v1.json`)
    that are `cp`-mirrored into `server/internal/sec/embedded/`. Update each to the
    new root path (`common/security/...`, `swarm/sdk/schemas/...`); the `//go:embed`
    targets remain the *local* `embedded/` copies (unchanged). The parity test
    proves the embedded copy still matches its (now-moved) canonical source.
  - **Comments.** Doc-comment `ai/` references in `main.go`, `bus/envelope.go`,
    `config.go`, `sync_test.go`, `sec/doc.go`, `sec/patterns.go`, `sec/embed.go`
    swept to the new paths.
  - `go build ./server/...` and `go test ./server/internal/sec/...` (parity) must
    pass after the changes (ledger #31).

- [x] **k8s manifests** (`infra/k8s/`): every `PYTHONPATH=ai`, `python ai/`,
  and volume mount referencing `/app/ai` corrected. `xops/lint/phase22_k8s_paths.py`
  scanner added as a CI step (exits non-zero on any `ai/` reference).

- [x] **All `.github/workflows/*.yml`**: `PYTHONPATH` overrides updated; `working-directory: ai` removed; all action pins remain at SHA (Phase 18 §18.2).

- [x] **Makefile**: all `PYTHONPATH=ai` occurrences replaced with `PYTHONPATH=.`;
  `make test.ai` updated to `PYTHONPATH=. pytest tests/` (old target kept as
  deprecated alias emitting a one-line message for one release cycle);
  `ai_commands.py` dispatcher updated or removed.

- [x] **`xops/makefile/ai_commands.py`** updated or deprecated: this dispatcher
  was the entry point for AI pipeline commands; post-migration it becomes a
  thin shim or is replaced by per-package dispatchers.

- [x] **`docker-compose.yml`** and `docker-compose.chaos.yml`: any `PYTHONPATH=ai`
  env vars or `ai/`-mounted volumes updated; compose re-validated with
  `make up PROFILES=core,server,swarm` exiting 0.

- [x] **`pyproject.toml`**: `testpaths = ["tests", "common/tests", "swarm/tests",
  "server/tests"]`; `pythonpath = ["."]` confirmed; `known_first_party` reflects
  all root packages; `coverage.run source` updated; no `"ai"` anywhere.

- [x] Root `conftest.py` `sys.path` cleanup: the `_phase22_layout` branch is now
  the only branch (the `ai/` layout detection is removed because `ai/` no longer
  exists). The `_phase22_layout` guard and the `else` branch are removed; the
  root conftest becomes unconditional.

- [x] **`xops/lint/phase22_k8s_paths.py`** wired into CI as a blocking step.

- [x] Proof tests: `test_22_5_no_ai_dockerfile_exists.py`, `test_22_5_root_dockerfile_builds_successfully.py`, `test_22_5_no_pythonpath_ai_in_makefile.py`, `test_22_5_no_pythonpath_ai_in_ci_workflows.py`, `test_22_5_k8s_manifests_no_ai_paths.py`, `test_22_5_docker_image_size_within_tolerance.py`, `test_22_5_make_test_ai_alias_emits_deprecation.py`, `test_22_5_go_build_succeeds_after_path_migration.py`, `test_22_5_go_runtime_path_const_updated.py`, `test_22_5_go_parity_table_source_paths_updated.py`, `test_22_5_go_sec_parity_test_passes_after_move.py`, `test_22_5_conftest_is_unconditional.py`.

---

### 22.6 — Config single-source consolidation

`common/config/defaults.yaml` (moved from `common/defaults.yaml` in §22.3a)
becomes the **single source** for all default values, shared by Python
(`common/config/ai_pipeline.py`) and Go (`server/internal/config/config.go`).

- [x] **`CURRENT_SEASON` migration** (ledger #29): every `CURRENT_SEASON` literal
  in the moved `constants.py`, `config.py`, and any module that imported the
  constant is replaced by `cfg.current_season` (read from `common/config/defaults.yaml`).
  `xops/lint/no_hardcoded_season.py` asserts zero bare `CURRENT_SEASON` literals
  outside `common/config/`.

- [x] **`N_FEATURES` is derived, never hardcoded, and Phase 22 does not change its
  value** (ledger #30). `common/constants.py` carries
  `N_FEATURES: int = len(FEATURE_COLUMNS)`. **There is a live inconsistency to
  reconcile, not propagate:** the comment in `constants.py` currently reads
  “Auto-resolves to 147 after Phase 21.16 lands,” while the Phase 21 canonical
  DoD (§21.26) states the final value is **157** (120 + 10 QID + 27 enrichment).
  Phase 22 **moves the file unchanged** and reconciles only the stale *comment*
  to match the actual `len(FEATURE_COLUMNS)` that Phase 21 left in place — it
  asserts the derived identity, never a literal. If `len(FEATURE_COLUMNS)` does
  not equal the Phase 21 canonical value at migration time, that is a Phase 21
  defect surfaced here, recorded as a tracker row, and fixed in Phase 21’s
  scope, not silently “fixed” by editing the constant during the move.

- [x] **`make config.export-env`** generates `xops/env/.env.example` from
  `common/config/defaults.yaml` + Config field metadata. A `# GENERATED BY make
  config.export-env — do not edit directly` header is included.

- [x] **`make config.export-doc`** generates the registry section of
  `docs/design/CONFIGURATION.md` between `<!-- BEGIN GENERATED -->` /
  `<!-- END GENERATED -->` fences. Narrative prose stays hand-edited.

- [x] CI runs both generators in a check-only mode (`--check`) and asserts `git
  diff --exit-code` on outputs; drift fails the build.

- [x] **Go config synchronisation**: `server/internal/config/config.go` reads
  the same `common/config/defaults.yaml` (via a generated `config_defaults.go`
  file produced by `make config.export-go`) so Python and Go share one source of
  truth. The Go file is regenerated whenever `defaults.yaml` changes; CI verifies
  the two are in sync.

- [x] All Phase 21 enrichment tunables round-trip through the generator: present
  in `defaults.yaml`, present in `xops/env/.env.example`, present in the
  `docs/design/CONFIGURATION.md` registry.

- [x] `common/` path references in `common/config/ai_pipeline.py` are zero:
  the file reads only from `common.*` sub-packages.

- [x] Proof tests: `test_22_6_env_example_generated_matches_defaults.py`, `test_22_6_configuration_doc_generated_matches_defaults.py`, `test_22_6_go_config_generated_matches_defaults.py`, `test_22_6_no_hardcoded_current_season.py`, `test_22_6_n_features_is_len_feature_columns_not_literal.py`, `test_22_6_n_features_matches_phase21_canonical_value.py`, `test_22_6_n_features_stale_comment_reconciled.py`, `test_22_6_config_round_trip_all_enrichment_keys.py`, `test_22_6_config_exports_are_idempotent.py`.

---

### 22.7 — `ai/` shim layer and tree removal

> 🛑 **Hard-gated by the executable check, not by a ticked box.**
>
> `make isolation.shims-only` must exit 0 — every remaining file under `ai/`
> is a pure re-export shim (no classes, functions, or variable assignments).
> A ticked checkbox, tracker row, or ROADMAP prose item is NOT evidence. The
> command is evidence. Do not start this step until the command is green.

**Four pre-condition signals** must ALL be recorded in
`docs/tracking/phase18_shim_deletion_acks.md` before deletion:
1. Zero `ai.*-shim` `DeprecationWarning` emissions in CI for ≥ 14 days across
   ≥ 50 pipelines.
2. Zero hits in production runtime shim logs (`make shim.runtime.report`
   returns empty).
3. CODEOWNERS ACK from all four component owners (`datasource`, `swarm`,
   `server`, `common`) checked into the acks file.
4. Zero `ai/` references in Phase 17 patcher bundle storage
   (`make patcher.bundle.scan-ai-refs` exits 0 — Phase 19 §19.1 satisfied this
   as a prerequisite; re-verify here).

- [x] `make isolation.shims-only` exits 0.
- [ ] All four pre-condition signals recorded with timestamps.
- [ ] `git rm -r ai/` — tree deleted in a single atomic commit.
  The commit message: `phase22: delete ai/ shim tree [phase22-deletion]`.
- [ ] `test_ai_tree_gone.py` (already exists; designed to fail until this step)
  is now **green**.
- [ ] `xops/lint/ai_tree_resurrection.py` remains active in CI and blocks any
  future `ai/` re-creation.
- [ ] `import ai` raises `ModuleNotFoundError` — verified by
  `test_22_0_import_ai_fails_after_deletion.py`.
- [ ] `make isolation.snapshot.refresh` run immediately after deletion;
  `make isolation.check --full` exits 0 with the deletion commit.
- [ ] Proof tests: `test_ai_tree_gone.py` (now passes), `test_22_0_import_ai_fails_after_deletion.py`, `test_22_7_shim_window_pre_conditions_all_four_recorded.py`, `test_22_7_resurrection_lint_still_active.py`.

---

### 22.8 — Mock absorption and compose cleanup

- [x] **`docker-compose.mock.yml` removal** (Phase 18 §18.4 stub was shipped;
  usage-report must show zero hits across all envs over
  `cfg.compose_overlay_deprecation_days` before deletion):
  - `make compose.overlay.usage_report` exits 0 with zero hits.
  - File removed; mock is a compose profile in the base `docker-compose.yml`.

- [x] `server` binary `MODE=mocksrv` works without the separate compose file;
  `mocksrv` CLI alias retired with a deprecation message for one release cycle.

- [x] `make up PROFILES=core,mock` verified working with the new root-level layout.

- [x] **`datasource/` absent from repo root**: `test_22_4_no_datasource_folder_at_root.py`
  green (already green from §22.4; re-asserted here as a regression check).

- [x] `make up PROFILES=all` brings every service up without `docker-compose.mock.yml`.

- [x] Proof tests: `test_22_8_compose_mock_overlay_removed.py`, `test_22_8_server_mode_mocksrv_works_without_overlay.py`, `test_22_8_make_up_all_without_mock_overlay.py`, `test_22_8_no_datasource_folder_regression.py`.
...
- [x] **`make phase22.metric-scan`** (`xops/makefile/phase22.py::cmd_metric_scan`)
  scans all `TelemetrySink.register_*` calls, `prometheus_client.Counter/Gauge/
  Histogram` constructor calls, and any `_metrics[...]` dict initialisation
  patterns across the **pre-migration** `ai/` tree. Emits
  `docs/tracking/phase22_metric_rename_table.md` listing every non-conforming
  name with its proposed rename.

- [x] Non-conforming metric names are renamed **before** the owning module moves
  (in a pre-move cleanup commit for that package). The rename commit and the
  package-move commit are separate to keep the diff reviewable.

- [x] **Migration table** (`docs/tracking/phase22_metric_rename_table.md`) covers
  every non-conforming name; every renamed metric has a corresponding Grafana
  panel `expr` update in the same commit.

- [x] Alert rules in `xops/monitoring/*.yaml` updated in lockstep with metric
  renames; Phase 21 enrichment alert rules (§21.30) updated in the same step.

- [x] **30-day dual-emission window**: after each rename, the owning module emits
  BOTH the old name and the new name using `TelemetrySink.register_alias(old_name,
  new_name)` for `cfg.metric_rename_alias_days` (default 30). After the window,
  the alias is removed and the old name stops being emitted. Grafana dashboards
  consume the new name from day one of the window; the dual emission prevents
  alert gaps during dashboard updates that lag the rename.

- [x] All patcher metrics (Phase 17) verified to use component-namespace labels,
  not file-path-derived prefixes — these are unaffected and require no rename.

- [x] Proof tests: `test_22_9_no_ai_prefixed_metric_names.py`, `test_22_9_metric_names_are_string_literals_not_dynamic.py`, `test_22_9_grafana_dashboards_use_new_metric_names.py`, `test_22_9_alert_rules_use_new_metric_names.py`, `test_22_9_dual_emission_active_within_window.py`, `test_22_9_dual_emission_removed_after_window.py`, `test_22_9_metric_rename_table_covers_all_violations.py`.

---

### 22.10 — Versioning finalization and 1.0.0 promotions

Phase 18 §18.5 gates each `≥ 1.0.0` bump: `PUBLIC_API.md` +
`test_public_api_compat.py` + deprecation policy doc must all exist before
`make version.bump --to-1.0.0` runs.

- [x] `xops/versioning/chart.json` — `ai` chart key marked `eol` immediately
  after the §22.7 deletion commit.

- [x] `source_watcher` alias EOL: `datasource_watcher` is the **canonical** key and
  `source_watcher` is the **old alias** going `eol` in `chart.json` (verified
  2026-06-20: both keys currently coexist). Mark `source_watcher` `eol`; verify
  `make track.show COMPONENT=source_watcher` still returns historical tracker
  rows via the CLI translation layer (ledger #12).

- [x] For each component reaching `≥ 1.0.0`: commit `<component>/PUBLIC_API.md`
  (auto-generated from `__all__` by `make docs.api`),
  `<component>/tests/test_public_api_compat.py` (pins the public surface), and
  a section in `docs/coding/component_versioning.md` declaring the deprecation
  policy. `xops/versioning/version.py` refuses the bump without all three.

- [x] Components bumped to `1.0.0` in this phase (using the **real `chart.json`
  keys** — ledger #39):
  - `datasource_scraper` (package now physically at `scraper/`)
  - `datasource_refresher`
  - `datasource_emitter`
  - `datasource_watcher` (canonical; `source_watcher` is its `eol` alias)
  - `swarm`
  - `common`
  - `common_feeds` (the moved `common/feeds/` → `common/feeds/`)

- [x] **Deferred to Phase 17 (not bumped here):** `datasource_patcher` and
  `datasource_gitops` — their packages do not exist at Phase 22 time (ledger #38);
  their `1.0.0` bumps ship with Phase 17.
- [x] **Owned by Phase 21 (not bumped here):** the four `enrichment_*` plane keys
  reach `1.0.0` per Phase 21 §21.26 DoD. There is **no single `enrichment` chart
  key** to bump (ledger #39); the `datasource/enrichment/` → `enrichment/`
  move is a path change, not a versioned-component creation.

- [x] `make version.validate` exits 0 after every individual bump.

- [x] Proof tests in `tests/test_22_10_versioning_finalization.py` (7 tests, all passing):
  - ✅ `test_22_10_ai_chart_key_eol`
  - ✅ `test_22_10_source_watcher_alias_eol`
  - ✅ `test_22_10_each_1_0_0_component_has_public_api_doc`
  - ✅ `test_22_10_each_1_0_0_component_has_compat_tests`
  - ✅ `test_22_10_version_bump_list_uses_real_chart_keys`
  - ✅ `test_22_10_patcher_gitops_bumps_deferred_to_phase17`
  - ✅ `test_22_10_version_chart_round_trips`

---

### 22.11 — Dead-code elimination and schema pruning

- [x] **Dead-code candidates** from the §22.1 pre-flight `vulture` scan
  (`docs/tracking/phase22_dead_code_candidates.md`) are reviewed. Confirmed
  dead symbols are removed in dedicated cleanup PRs (not mixed with package moves)
  with tests asserting no live callers exist. Symbols that are legitimately private
  are annotated `# vulture: whitelist <reason>`.

- [x] **v0.2 proxy column annotations** (`# DEPRECATED: superseded by enrichment
  plane <N>` in `model/features.py`) verified present after the `model/`
  move to `model/`.

- [x] **JSON schema pruning**: `common/schemas/feeds/` (merged from `common/schemas/feeds/`)
  contains Phase 21 enrichment schemas plus original feed schemas.
  `make schema.validate` must pass with zero warnings; any duplicate schema files
  from the merge must be resolved.

- [x] **`__init__.py` audit**: every moved package's `__init__.py` declares an
  explicit `__all__`. The Phase 18 §18.13 gate
  `test_components_declare_public_api_via_all.py` runs against all new root-level
  packages; zero failures.

- [x] **`mypy --strict` pass** on all root packages (`scraper`, `model`, `nlp`,
  `pipeline`, `qid`, `tqu`, `trc`, `proofreader`, `orchestrator`, `backtest`,
  `enrichment`). Any type errors introduced by the merge are resolved. Mypy strict
  mode is only required for newly moved packages that had passing strict in `ai/`;
  packages that were previously excluded from strict remain excluded.

- [x] **SBOM final verification**: the per-component lock/SBOM regeneration
  already happened in each move commit (§22.4 step 4, ledger #37). This step is
  the post-merge audit: `make sbom.gen COMPONENT=swarm` and
  `make sbom.gen COMPONENT=enrichment` produce **no diff** (proving the in-commit
  regen was complete), and the Phase 18 §18.10 SBOM-diff gate is green across the
  whole tree. There is **no `datasource` SBOM component** — the old
  `datasource/` SBOM is superseded by `enrichment/`.

- [x] Proof tests: `test_22_11_dead_code_scan_report_committed.py`, `test_22_11_no_confirmed_dead_symbols_at_root.py`, `test_22_11_v0_2_proxy_annotations_present_after_move.py`, `test_22_11_all_root_packages_have_explicit_all.py`, `test_22_11_schema_validate_zero_warnings.py`, `test_22_11_mypy_strict_passes_on_moved_packages.py`, `test_22_11_sbom_updated_after_merge.py`.

---

### 22.12 — ROADMAP, design docs, and patcher artefact path sweep

- [x] **ROADMAP path reference sweep**: every occurrence of `patcher/`,
  `gitops/`, `common/`, `scraper/`, `swarm/`, etc. in
  `docs/planning/ROADMAP.md` §§17.1–17.9, §18.x, §19.x, §21.x is updated to
  the new root path. After the sweep, **re-validate every phase-ledger lint that
  actually exists** (`xops/lint/phase16_ledger.py` is present today; the
  `phase17`/`phase18` ledger lints are re-validated only if/when they exist —
  do not gate the sweep on a lint file that has not been written).

- [x] **Anchor design docs** (`docs/design/COMPONENT_LAYOUT.md`, `DATA_PIPELINE.md`,
  `EMITTER.md`, `SCRAPER_PATCHER.md`, `SECURITY.md`, `TESTING_STRATEGY.md`):
  all `ai/` sub-paths updated; `last_verified_against_code` front-matter bumped
  to the Phase 22 migration date; `make docs.verify` exits 0.

- [x] **`docs/guides/SETUP.md`**: all developer setup instructions updated to
  reflect `PYTHONPATH=.` and flat root layout.

- [x] **`AGENTS.md`, `CLAUDE.md`, `.github/copilot-instructions.md`**: all `ai/`
  path references updated; these are forbidden-edit files without an explicit
  human request naming them, but the path sweep is explicitly part of Phase 22
  scope.

- [x] **Patcher cassette path migration — CONDITIONAL (ledger #38).** Branch on the
  §22.1 `make phase22.patcher-artifact-probe` result:
  - **If `xops/patcher/cassettes/` is absent or empty** (the expected state —
    Phase 17 ships *after* Phase 22): assert empty and record the no-op in
    `docs/tracking/phase22_patcher_artifact_state.txt`. Do **not** create the
    directory or invent cassettes.
  - **If cassettes exist** (Phase 17 already landed): `make patcher.cassette.migrate`
    rewrites every `ai/` path string in cassette tool-call outputs
    (`Read: ai/...` → `Read: scraper/...`), re-signs each HMAC-SHA256 sidecar,
    updates `xops/patcher/cassettes/MANIFEST.sha256`, and `make patcher.cassette.drift`
    confirms no structural drift.

- [x] **Bundle metadata migration — CONDITIONAL (ledger #38).** Same branch on
  `feeds/ops/bundles/`:
  - **If absent** (expected): assert empty, record the no-op.
  - **If present**: `make patcher.bundle.migrate-paths` rewrites `ai/*` module
    paths in `failing_code_path` / `failing_code_excerpt` / scope-path fields
    (path string only; payload bytes unchanged) and re-computes the bundle
    envelope HMAC (per Phase 17 §17.2b `patcher/bundle_sign.py`).

- [x] `docs/design/COMPONENT_LAYOUT.md` §§3, 5, 7 rewritten to document the
  flat root layout (no `datasource/`, no `ai/`, no `datasource/`).

- [x] Proof tests: `test_22_12_roadmap_no_ai_path_backtick_references.py`, `test_22_12_present_phase_ledger_lints_pass_after_update.py`, `test_22_12_docs_verify_passes_after_sweep.py`, `test_22_12_patcher_migration_conditional_on_artifact_presence.py`, `test_22_12_patcher_no_op_recorded_when_phase17_unshipped.py`, `test_22_12_cassette_paths_migrated_and_sidecar_valid_when_present.py`, `test_22_12_bundle_metadata_paths_migrated_and_resigned_when_present.py`, `test_22_12_anchor_docs_last_verified_updated.py`, `test_22_12_cassette_drift_probe_clean_after_migration_when_present.py`.
...
The Phase 14 dashboard has a "Phase 22 Burn-In" panel showing all five counters
with a single green/red status.
