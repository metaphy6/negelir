...
- [ ] CLI entry point `xops/opsctl/__main__.py` invoked via `make ops.<cmd>` shortcuts (`ops.denylist-clear`, `ops.baseline-reset`, `ops.quarantine-erase`, `ops.quarantine-clear`, `ops.retrain-approve`, `ops.scale`, `ops.scale-pin`, `ops.scale-unpin`, `ops.backup-now`, `ops.dlq-replay`, `ops.allowlist-extend`, `ops.allowlist-approve`).
- [ ] Each subcommand:
...
- [ ] **Nightly dump.**
  ...
  - **PII-aware dump policy.** `quarantine_samples.raw_bytes_b64` is **excluded** from the nightly dump via `--exclude-table-data=quarantine_samples` and re-included as a sanitized projection (`(quarantine_id, source, rule_id, hit_fingerprint, created_at, erased_at)`) written to `negelir.quarantine_meta.csv` alongside the dump. Right-to-erasure invariant: a backup older than the quarantine TTL never contains pre-erasure raw bytes (Phase 16 GDPR-style stance per [`design/SECURITY.md`](../design/SECURITY.md)).
  - Optional encryption-at-rest: if `cfg.maint_backup_encryption_key_dir` (a **directory** of versioned recipient keys named `keys.vN.age.pub`) is set, the dump is encrypted with `age -R <newest>.pub`. Old keys are kept on disk for restore — **never deleted automatically**. Key rotation is operator-driven: `ops.backup-rotate-key` adds a new `vN+1` and re-emits a `maint.event.v1{kind=backup_key_rotated, prev_v=N, new_v=N+1}` event. If the dir is unset, dump is left unencrypted with a stderr warning + `sec.alert.v1{kind=backup_unencrypted, severity=warn}` (debounced daily).
...
- [ ] **Restore-verify (mandatory daily, runtime-aware).**
  - **Mechanism is pluggable.** A `RestoreVerifier` Protocol with two implementations:
    ...
    - `SidecarVerifier` (Phase 14): a Kubernetes `Job` template (committed at `infra/k8s/jobs/restore-verify.yaml`) is created via the API server using a least-privilege ServiceAccount (`create` on `jobs` only, scoped to a dedicated namespace). The agent watches the Job to completion, scrapes its `pg_restore` exit code + verify-suite output, then deletes the Job (and its PVC).
...
- [ ] **Two-mode insertion.**
  - `cfg.sec_input_pattern_autolearn_enabled = true`: insert into `pattern_allowlist` with `state='active', expires_at = now() + cfg.maint_sec_allowlist_ttl_days, added_by_request_id`. Emits `kind=pattern_allowlist_added`.
  - `cfg.sec_input_pattern_autolearn_enabled = false` (default): insert into `pattern_allowlist` with `state='pending', expires_at = NULL, proposed_at = now()`. Emits `kind=pattern_allowlist_pending` (informational; **does not** count as `added`). Operator promotes via `ops.allowlist-approve --id <row_id> --ttl-days N` → row flips to `state='active'` and a `kind=pattern_allowlist_added` event fires. Pending rows older than `cfg.maint_sec_allowlist_pending_ttl_days` (default 30) are pruned by §8.3.
- [ ] `sec.input.v1` reads only `state='active' AND expires_at > now()` rows on each pattern hit — if `(source, rule_id, hit_fingerprint)` matches, the hit is suppressed (counted in `sec_input_allowlist_hits_total` for visibility, never silently lost). The cache is refreshed via mtime-equivalent on a `pattern_allowlist_version` row in a metadata table (poll interval `cfg.sec_input_allowlist_reload_s`, default 60s) — same pattern as §7.4 sec.config fan-out (deliberately mtime-style, not bus fan-out).
...
- [ ] **Hysteresis.** No more than one decimate per `cfg.maint_sec_decimate_min_interval_s` (default 300s) **globally** (single zset, single global pressure signal — per-subject hysteresis is meaningless for a global cap). Tracked in agent state, not Redis (state survives within a single agent process; `replicas: 1` enforced).
...
- [ ] All 20 new `maint.event.v1.kind` values (per the §3.5 list above) join the §7.4 known-kinds set; producer-set allow-list updated; boundary test green.
...
- [ ] All Phase 8 cfg knobs (`maint_runtime`, `maint_scaler_*` ×11 incl. `pin_max_ttl_s`/`vram_headroom_mb`/`runtime_timeout_s`/`window_ns` derived/`history_max`, `maint_backup_*` ×12 incl. `max_skew_h`/`age_alert_h`/`prune_batch`/`prune_max_lock_ms`/`verify_mode`/`encryption_key_dir`, `maint_dlq_*` ×8 incl. `replay_topics_allow`/`backlog_alert`/`max_replays_per_tick`, `maint_schema_*` ×4 incl. `sample_rate`, `maint_sec_*` ×3 incl. `allowlist_pending_ttl_days`, `maint_audit_retention_days`, `maint_silence_alert_h`, `opsctl_ack_timeout_ms`, `opsctl_spool_max_entries`, `source_watcher_summarizer_*` ×2) appear in `xops/env/.env.example`, `ai/common/defaults.yaml`, and `ai/common/config.py`. `test_config_sync.py` stays green.

**Migrations:**

- [ ] `migrations/009_maint_audit.sql` lands the `maint_audit_log` table (additive — no DROP, per CLAUDE.md doctrine). Includes `pattern_allowlist_version` metadata row scaffolding for §8.7 mtime-style cache reload.
- [ ] `migrations/010_pattern_allowlist.sql` lands the `pattern_allowlist` table (additive) with `state CHAR(1) CHECK (state IN ('p','a','e'))` (pending/active/expired), `proposed_at`, `expires_at`, `added_by_request_id`, indexed on `(source, rule_id, hit_fingerprint, state)`.
- [ ] Both migrations apply cleanly on a fresh DB and as upgrades from migration 008. Migration round-trip test runs Detector B against the post-migration schema and asserts zero false-positive drift.
...
- [ ] **Leader-election (Phase 14, binding for `replicas: 1` agents).** `maint.scaler.v1`, `maint.backup.v1`, and `maint.schema.v1` use the standard K8s `coordination.k8s.io/v1.Lease` client (15s lease, 10s renew, 5s retry). On lease loss, the agent enters "observer mode" — continues consuming for ledger warmth but **does not publish**. Compose mode: only one replica per service is ever started (compose enforces). The leader-election adapter is a `Leader` Protocol with `KubernetesLeader` and `SingleProcessLeader` implementations; agent code calls `if not self.leader.is_leader(): return` before every emit. Boundary test asserts every emit path is gated.
- [ ] **Dead-mans-switch.** A dedicated `maint.heartbeat.v1` (no — reuse `telemetry.v1{kind=maint_silence_check}`) periodic check: if `now − last_observed_maint_event_v1 > cfg.maint_silence_alert_h` (default 24h) AND the swarm has been up `> 1h` (boot-warm guard), emit `sec.alert.v1{kind=maint_silence_alert, severity=critical}` (bypass debounce). Catches the worst silent-failure mode: the maint plane is dead so it can't tell you it's dead. The check itself runs in **`telemetry.v1`** (already always-on; §8.x can't be both judge and patient).
