...
| 11 | **Phase persistence — do not stop mid-phase (binding on every model)** | When the human asks you to implement / finish / complete a phase, sub-phase, or slice, the work is the **entire named scope**. Loop internally over every `- [ ]` bullet in that scope (in `docs/planning/ROADMAP.md` and the matching `docs/design/*.md` checklists) until they are all `[x]` or a *real* blocker is hit. **This rule applies equally to Claude Sonnet/Opus, GPT-5 / 5-Codex, Gemini, and any other backend** — there is no "smaller model" exception. Models in the GPT and Gemini families have a measured tendency to return partial work and ask "should I continue?"; that behaviour is a Rule 11 violation, not polite engineering. Real blockers are narrow: cross-phase forbidden-edit, doctrine conflict, an actually-stuck failing test, a DoD item that requires a human decision (operator key, prod credential), or the human capped scope in the request. **Not** blockers: "this is large", "many edits", "context tight", "shall I continue?", "I finished part X — proceed with Y?", "I'll pause for review", "returning control to confirm direction". If you catch yourself drafting any of those sentences, delete them and start the next bullet. Per-bullet bookkeeping (tracker row + version bump + checkbox flip per §3.4 / §6.1) still happens for every bullet — you batch the *work*, not the bookkeeping. Final summary once the phase is genuinely drained: list every bullet closed, every command you ran, and any `[ ]` still open with the explicit doctrine reason. Then hand back for `make git`. The chat-mode equivalent of this rule lives in [`.github/copilot-instructions.md`](.github/copilot-instructions.md) §6 and binds every Copilot session by default; the CI equivalent lives in [`.github/instructions/ci-pipeline.instructions.md`](.github/instructions/ci-pipeline.instructions.md) and binds every orchestrator workflow run. |
