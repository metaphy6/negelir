...
class PredictRequest:
    ...
    Phase 9 RPC pattern can correlate the response. `kind_schema_version`
    keeps the additive `qa_correlation_id` wire change backward-compatible:
    legacy payloads omit the field (treated as version 1), new producers
    emit version 2.
    """
    ...
    requested_at: str = ""
    kind_schema_version: int = 2
    qa_correlation_id: str | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.kind_schema_version not in (1, 2):
            raise ValueError(
                "PredictRequest.kind_schema_version must be 1 or 2"
            )
    ...
    def from_dict(cls, data: dict[str, Any]) -> "PredictRequest":
        return cls(
            ...
            requested_at=str(data.get("requested_at", "")),
            kind_schema_version=int(data.get("kind_schema_version", 1)),
            qa_correlation_id=(
                None if data.get("qa_correlation_id") is None
                else str(data.get("qa_correlation_id"))
            ),
...
KNOWN_SEC_ALERT_KINDS: frozenset[str] = frozenset({
    ...
    # maint.scaler.v1 (§8.16.8 unknown per-model VRAM footprint fallback)
    ...
    # sec.input.v1 (§8.16.10 — allowlist HMAC key age exceeded the
    # configured rotation max age; warn, debounced daily).
    "allowlist_hmac_key_rotation_overdue",
})
...
class SecAlert:
    ...
    def publish_topics(*, kind: str, severity: str, target: str | None) -> tuple[str, ...]:
        """Return the canonical publish-topic tuple for the given alert shape."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.event_correlation_id is None:
            try:
                ...
                topics = ()
            if MAINT_EVENT_TOPIC in topics and SEC_ALERT_TOPIC in topics:
                object.__setattr__(
                    self,
    ...
    def from_dict(cls, data: dict[str, Any]) -> "SecAlert":
        return cls(
            ...
            event_correlation_id=data.get("event_correlation_id"),
        )
