"""Phase 19.9 — Isolation policy enforcement for datasource extractors (Phase 22.3b symbol-union merge).
...
Phase 22.3b merge: Root base + AI-unique symbols (IsolationViolation dataclass,
IsolationPolicy class, validate_extractor_isolation).
"""
...
class IsolationViolation:
    """Represents an isolation policy violation (AI-unique dataclass)."""
    extractor_path: str
...
class IsolationPolicy:
    """Enforces component isolation boundaries (AI-unique class)."""
...
def validate_extractor_isolation(extractor_path: str) -> List[IsolationViolation]:
    """Validate that a new extractor obeys isolation policy (AI-unique).
